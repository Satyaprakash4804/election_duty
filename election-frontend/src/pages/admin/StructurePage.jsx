...
function StaffPickerSheet({ allowedRanks, color, onPick, onClose }) {
  ...
  const load = useCallback(async (reset=false, qVal=q, rf=rankFilter) => {
    ...
    try {
      let url = `/admin/staff?assigned=no&page=${pageRef.current}&limit=20&q=${encodeURIComponent(qVal)}`;
      if (rf) url += `&rank=${encodeURIComponent(rf)}`;
      const res = await adminApi.getStaff ? 
        await (async () => { const r = await fetch(`/api${url}`, { headers: { Authorization: `Bearer ${token}` } }); return r.json(); })()
      ...
      // fallback: try adminApi
      try {
        const res = await adminApi.getStaff({ assigned: 'no', page: pageRef.current-1, limit: 20, q: qVal, rank: rf || undefined });
        const w = res.data?.data?.data || res.data?.data || [];
...
function ItemDialog({ title, color, Icon: I, fields, officerTitle, officerRanks, existing, createUrl, updateUrlFn, onDone, onClose }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (existing) {
        await fetch(`/api${updateUrlFn(existing.id)}`, { method:'PUT', headers, body: JSON.stringify(body) });
      } else {
        await fetch(`/api${createUrl}`, { method:'POST', headers, body: JSON.stringify(body) });
      }
      onDone();
      onClose();
...
function MatdanSthalDialog({ centerId, centerName, onClose }) {
  ...
  const loadRooms = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/centers/${centerId}/rooms`, { headers: token ? { Authorization:`Bearer ${token}` } : {} });
  ...
  const addRoom = async () => {
    ...
    setAdding(true);
    try {
      await fetch(`/api/admin/centers/${centerId}/rooms`, {
        method:'POST', headers:{'Content-Type':'application/json', ...(token?{Authorization:`Bearer ${token}`}:{})},
        body: JSON.stringify({ roomNumber: roomNum.trim() }),
      });
      setRoomNum('');
      await loadRooms();
    } catch (e) { toast.error(`Error: ${e.message}`); }
  ...
  const deleteRoom = async (roomId) => {
    try {
      await fetch(`/api/admin/rooms/${roomId}`, { method:'DELETE', headers: token?{Authorization:`Bearer ${token}`}:{} });
      await loadRooms();
    } catch (e) { toast.error(`Error: ${e.message}`); }
  };
    ...
    <div>
      ...
      {assigningRank && (
        <StaffPickerSheet allowedRanks={[assigningRank, ...RANK_HIERARCHY.slice(RANK_HIERARCHY.indexOf(assigningRank)+1)]} color={C.primary}
          onPick={async s => {
            try {
              await fetch(`/api/admin/duties`, { method:'POST', headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})}, body: JSON.stringify({ staffId:s.id, centerId }) });
...
function CenterDialog({ gpId, existing, onDone, onClose }) {
  ...
  const runAutoAssign = async (centerId, { isReassign=false, rules=null }={}) => {
    ...
    try {
      const headers = { 'Content-Type':'application/json', ...(token?{Authorization:`Bearer ${token}`}:{}) };
      if (isReassign) {
        try { await fetch(`/api/admin/centers/${centerId}/clear-assignments`, { method:'POST', headers, body:'{}' }); } catch(_) {}
      }
      const body = {};
      if (rules?.length) body.customRules = rules.map(r=>({ rank:r.rank, count:r.count }));
      const res = await fetch(`/api/admin/auto-assign/${centerId}`, { method:'POST', headers, body: JSON.stringify(body) });
      const data = await res.json();
      const d = data.data || {};
  ...
  const save = async () => {
    ...
    try {
      ...
      if (existing) {
        await fetch(`/api/admin/centers/${existing.id}`, { method:'PUT', headers, body: JSON.stringify(body) });
        setSaving(false);
        ...
        const res = await fetch(`/api/admin/gram-panchayats/${gpId}/centers`, { method:'POST', headers, body: JSON.stringify(body) });
        const data = await res.json();
        const centerId = data.data?.id || 0;
        setSaving(false);
        if (centerId > 0) {
...
function StepList({ title, Icon: I, color, officerTitle, officerRanks, fetchUrl, createUrl, updateUrlFn, deleteUrlFn, fields, onSelect, selectedId }) {
  ...
  const [page, setPage] = useState(1);
  const [q, setQ] = useState('');
  const [dialog, setDialog] = useState(null); // null | 'add' | Map (existing)
  ...
  const load = useCallback(async (reset=false) => {
    ...
    try {
      const url = `${fetchUrl}?page=${pageRef.current}&limit=${LIMIT}&q=${encodeURIComponent(qRef.current)}`;
      const res = await fetch(`/api${url}`, { headers: token?{Authorization:`Bearer ${token}`}:{} });
      const data = await res.json();
      let fetched = [], totalPages = 1;
      if (Array.isArray(data.data)) { fetched = data.data; }
      else if (data.data?.data) { fetched = data.data.data; totalPages = data.data.totalPages || 1; }
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`/api${deleteUrlFn(deleteTarget.id)}`, { method:'DELETE', headers: token?{Authorization:`Bearer ${token}`}:{} });
...
function CenterStep({ gpId }) {
  ...
  const load = useCallback(async (reset=false) => {
    ...
    try {
      const res = await fetch(`/api/admin/gram-panchayats/${gpId}/centers?page=${pageRef.current}&limit=20&q=${encodeURIComponent(qRef.current)}`, { headers:token?{Authorization:`Bearer ${token}`}:{} });
      const data = await res.json();
      let items=[], totalPages=1;
      if (Array.isArray(data.data)) { items=data.data; }
      else if (data.data?.data) { items=data.data.data; totalPages=data.data.totalPages||1; }
      hasMoreRef.current = pageRef.current < totalPages;
      setHasMore(pageRef.current < totalPages);
      pageRef.current++;
  ...
  const handleDelete = async () => {
    try {
      const {token} = useAuthStore();
      await fetch(`/api/admin/centers/${deleteTarget.id}`, { method:'DELETE', headers:token?{Authorization:`Bearer ${token}`}:{} });
