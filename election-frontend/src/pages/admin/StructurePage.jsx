...
import {
  ...
  Plus, Pencil, Trash2, Search, X, Check, AlertTriangle, Info,
  User, Badge, Phone, ShieldCheck, DoorOpen, ListChecks,
  ArrowDownUp, CheckCircle2, XCircle, Lock, Unlock,
  RefreshCw, ClipboardList, Ban
} from 'lucide-react';
import { adminApi } from '../../api/endpoints';
...
const C = {
  bg: '#FDF6E3', surface: '#F5E6C8', primary: '#8B6914', accent: '#B8860B',
  dark: '#4A3000', subtle: '#AA8844', border: '#D4A843',
  error: '#C0392B', success: '#2D6A1E', info: '#1A5276', orange: '#E65100',
};
...
// ══════════════════════════════════════════════════════════════════════════════
//  BACKGROUND JOB STATE — singleton map (szId → job state)
//  Survives re-renders because it lives outside React tree
// ══════════════════════════════════════════════════════════════════════════════
const activeJobs = {}; // szId → { szId, szName, jobId, status }
let jobListeners = [];  // callbacks to notify
...
function useJobStore() {
  ...
  useEffect(() => {
    ...
    return () => { jobListeners = jobListeners.filter(f => f !== fn); };
  }, []);
  return activeJobs;
}
...
function Spinner({ size = 18, color = 'white' }) {
  return (
    <div style={{
      ...
      border: `2px solid ${hex('#888888', 0.25)}`,
    ...
    <div style={{ marginBottom: 10 }}>
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
          style={{
            width: '100%', padding: `10px 12px 10px ${Icon ? 34 : 12}px`,
            background: 'white', border: `1.5px solid ${focused ? color : C.border}`,
            borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none',
            boxShadow: focused ? `0 0 0 3px ${hex(color, 0.12)}` : 'none',
            transition: 'all 0.2s', fontFamily: 'inherit',
...
// ── Confirm dialog ────────────────────────────────────────────────────────────
function ConfirmDialog({ open, message, onConfirm, onCancel, confirmColor = C.error, confirmLabel = 'हटाएं', title = 'Confirm Delete', Icon: I = AlertTriangle }) {
  if (!open) return null;
  return (
    <Modal open onClose={onCancel} maxWidth={380}>
      <ModalHeader title={title} Icon={I} color={confirmColor} onClose={onCancel} />
      <div style={{ padding: 20 }}>
        <p style={{ color: C.dark, fontSize: 13, marginBottom: 20 }}>{message}</p>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onCancel} style={{
          ...
          <button onClick={onConfirm} style={{
            flex: 1, padding: '10px 0', border: 'none', borderRadius: 10,
            background: confirmColor, color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700,
          }}>{confirmLabel}</button>
        </div>
      </div>
    </Modal>
...
// ══════════════════════════════════════════════════════════════════════════════
//  GLOBAL JOB BANNER — shown at top when any SZ assignment is running
// ══════════════════════════════════════════════════════════════════════════════
function GlobalJobBanner() {
  ...
  return (
    <div style={{
      background: hex(C.orange, 0.12), padding: '8px 12px',
      display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0,
      borderBottom: `1px solid ${hex(C.orange, 0.3)}`,
    }}>
      <Spinner size={14} color={C.orange} />
      <p style={{ flex: 1, color: C.orange, fontSize: 11, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {running.map(j => `${j.szName}: ${j.status === 'pending' ? 'शुरू हो रहा है...' : 'असाइन हो रहा है...'}`).join(' • ')}
      </p>
      <RefreshCw size={14} color={C.orange} />
    </div>
  );
}
...
function StaffPickerSheet({ allowedRanks, color, onPick, onClose }) {
  ...
  const load = useCallback(async (reset = false, qVal = q, rf = rankFilter) => {
    if (!reset && !hasMoreRef.current) return;
    if (reset) { pageRef.current = 1; hasMoreRef.current = true; setStaff([]); setLoading(true); }
    else setLoadingMore(true);
    try {
      const params = { assigned: 'no', page: pageRef.current, limit: 20, q: qVal };
      if (rf) params.rank = rf;
      ...
      pageRef.current++;
      setStaff(prev => reset ? w : [...prev, ...w]);
    } catch (e) { toast.error(`Staff load error: ${e.message}`); }
    finally { setLoading(false); setLoadingMore(false); }
  }, []);
  ...
  return (
    <Modal open onClose={onClose} maxWidth={480} maxHeight="80vh">
      <ModalHeader title="Staff से चुनें (अनसाइन)" Icon={User} color={color} onClose={onClose} />
      <div style={{ padding: '10px 16px 6px' }}>
        {allowedRanks.length > 0 && (
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 6, marginBottom: 8 }}>
            {[{ label: 'सभी', value: '' }, ...allowedRanks.map(r => ({ label: r, value: r }))].map(({ label, value }) => {
              ...
              return (
                <button key={value} onClick={() => { setRankFilter(value); load(true, q, value); }} style={{
                  padding: '5px 10px', borderRadius: 20, fontSize: 11, flexShrink: 0, cursor: 'pointer',
                  fontWeight: sel ? 700 : 500, background: sel ? rc : 'white',
                  color: sel ? 'white' : C.dark, border: `1px solid ${sel ? rc : hex(C.border, 0.5)}`,
                }}>
                  {label}
                </button>
              );
        ...
        <div style={{ position: 'relative' }}>
          <Search size={16} color={C.subtle} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)' }} />
          <input value={q} placeholder="नाम, PNO खोजें..."
            onChange={e => { setQ(e.target.value); debouncedSearch(e.target.value, rankFilter); }}
            style={{ width: '100%', padding: '9px 12px 9px 32px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', fontFamily: 'inherit' }} />
        </div>
      </div>
      <div ref={scrollRef} onScroll={handleScroll} style={{ flex: 1, overflowY: 'auto', padding: '0 16px 20px' }}>
        {loading ? (
          ...
          const rc = RANK_COLORS[s.rank] || C.primary;
          return (
            <div key={s.id} onClick={() => onPick(s)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: `1px solid ${hex(C.border, 0.2)}`, cursor: 'pointer' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, background: hex(rc, 0.12), border: `1px solid ${hex(rc, 0.3)}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: rc }}>
                {initials(s.name)}
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontWeight: 700, fontSize: 13, color: C.dark }}>{s.name}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {s.pno && <span style={{ fontSize: 11, color: C.subtle }}>{s.pno}</span>}
                  <span style={{ padding: '1px 6px', borderRadius: 5, fontSize: 10, fontWeight: 700, background: hex(rc, 0.1), color: rc, border: `1px solid ${hex(rc, 0.3)}` }}>{s.rank}</span>
                </div>
              </div>
              <button style={{ padding: '5px 10px', background: color, color: 'white', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>चुनें</button>
            </div>
          );
        })}
        {loadingMore && <div style={{ display: 'flex', justifyContent: 'center', padding: 12 }}><Spinner color={C.primary} size={16} /></div>}
...
function OfficerCard({ index, officer, color, allowedRanks, canRemove, onChange, onRemove }) {
  ...
  return (
    <div style={{ marginBottom: 10, border: `1px solid ${hasData ? hex(color, 0.3) : hex(C.border, 0.4)}`, borderRadius: 10, background: hasData ? hex(color, 0.04) : 'white', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 10px 10px 12px', cursor: 'pointer' }} onClick={() => setExpanded(e => !e)}>
        <div style={{ width: 28, height: 28, borderRadius: '50%', background: hex(color, 0.12), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 900, color, flexShrink: 0 }}>
          {index + 1}
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontWeight: 700, fontSize: 13, color: hasData ? C.dark : C.subtle }}>{hasData ? officer.name : `अधिकारी ${index + 1}`}</p>
          {hasData && officer.rank && <p style={{ fontSize: 11, color }}>{officer.rank}</p>}
        </div>
        <button onClick={e => { e.stopPropagation(); setShowPicker(true); }} style={{ padding: '4px 8px', background: hex(C.info, 0.08), border: `1px solid ${hex(C.info, 0.3)}`, borderRadius: 6, fontSize: 10, fontWeight: 700, color: C.info, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
          <Search size={11} /> Staff से चुनें
        </button>
        <span style={{ color: C.subtle, fontSize: 18 }}>{expanded ? '▲' : '▼'}</span>
        {canRemove && (
      ...
      {expanded && (
        <div style={{ padding: '0 12px 12px' }}>
          {allowedRanks.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <label style={{ fontSize: 11, color: C.subtle, fontWeight: 600, display: 'block', marginBottom: 4 }}>पद / Rank</label>
              <select value={officer.rank} onChange={e => onChange({ ...officer, rank: e.target.value })} style={{ width: '100%', padding: '9px 12px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', fontFamily: 'inherit', cursor: 'pointer' }}>
                <option value="">-- Rank चुनें --</option>
                {allowedRanks.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
          )}
          {[{ field: 'name', label: 'पूरा नाम *', icon: User }, { field: 'pno', label: 'PNO', icon: Badge }, { field: 'mobile', label: 'मोबाइल', icon: Phone, type: 'tel' }].map(({ field, label, icon, type = 'text' }) => (
            <FieldInput key={field} label={label} icon={icon} color={color} type={type} value={officer[field]} onChange={v => onChange({ ...officer, [field]: v })} />
          ))}
        </div>
      )}
      {showPicker && (
...
function ItemDialog({ title, color, Icon: I, fields, officerTitle, officerRanks, existing, createUrl, updateUrlFn, onDone, onClose }) {
  ...
  const [form, setForm]         = useState(() => { const f = {}; fields.forEach(fld => f[fld] = existing?.[fld] || ''); return f; });
  const [officers, setOfficers] = useState(initOfficers);
  const [saving, setSaving]     = useState(false);
  ...
  const save = async () => {
    ...
    try {
      const body = { ...form, officers: officers.filter(o => o.name.trim()).map(o => ({ ...o })) };
      if (existing) { await apiClient.put(updateUrlFn(existing.id), body); }
      else           { await apiClient.post(createUrl, body); }
      onDone(); onClose();
    } catch (e) { toast.error(`Error: ${e.message}`); }
    finally { setSaving(false); }
  };
  ...
  return (
    <Modal open onClose={onClose} maxWidth={520}>
      ...
      <div style={{ overflowY: 'auto', flex: 1, padding: 16 }}>
        {fields.map(f => {
          if (f === 'district') {
            return (
              <div key="district" style={{ marginBottom: 10 }}>
                <label style={{ fontSize: 11, color: C.subtle, fontWeight: 600, display: 'block', marginBottom: 4 }}>जिला</label>
                <select value={form[f]} onChange={e => setForm(p => ({ ...p, district: e.target.value }))} style={{ width: '100%', padding: '10px 12px', background: 'white', border: `1.5px solid ${color}`, borderRadius: 10, fontSize: 13, color: form[f] ? C.dark : C.subtle, outline: 'none', fontFamily: 'inherit', cursor: 'pointer' }}>
                  <option value="">-- जिला चुनें --</option>
          ...
          return <FieldInput key={f} label={fieldLabel(f)} icon={fieldIcon(f)} color={color} value={form[f]} onChange={v => setForm(p => ({ ...p, [f]: v }))} />;
        })}
        ...
        {officerRanks.length > 0 && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, marginTop: 6 }}>
              <div style={{ width: 3, height: 14, background: color, borderRadius: 2 }} />
              <p style={{ flex: 1, color, fontSize: 12, fontWeight: 800 }}>{officerTitle}</p>
              <button onClick={() => setOfficers(p => [...p, blankOfficer()])} style={{ padding: '4px 8px', background: hex(color, 0.1), border: `1px solid ${hex(color, 0.3)}`, borderRadius: 7, fontSize: 11, fontWeight: 700, color, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                <Plus size={12} /> जोड़ें
              </button>
            </div>
            {officers.map((o, i) => (
      ...
      <div style={{ padding: '8px 16px 16px', display: 'flex', gap: 12 }}>
        <button onClick={onClose} style={{ flex: 1, padding: '12px 0', border: `1px solid ${C.border}`, borderRadius: 10, background: 'transparent', color: C.subtle, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>रद्द</button>
        <button onClick={save} disabled={saving} style={{ flex: 1, padding: '12px 0', border: 'none', borderRadius: 10, background: color, color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: saving ? 0.7 : 1 }}>
          {saving ? <Spinner /> : 'सेव करें'}
        </button>
      </div>
...
function MatdanSthalDialog({ centerId, centerName, onClose }) {
  ...
  const addRoom = async () => {
    if (!roomNum.trim()) return;
    setAdding(true);
    try { await apiClient.post(`/admin/centers/${centerId}/rooms`, { roomNumber: roomNum.trim() }); setRoomNum(''); await loadRooms(); }
    catch (e) { toast.error(`Error: ${e.message}`); }
    finally { setAdding(false); }
  };

  const deleteRoom = async (roomId) => {
    try { await apiClient.delete(`/admin/rooms/${roomId}`); await loadRooms(); }
    catch (e) { toast.error(`Error: ${e.message}`); }
  };
  ...
  return (
    <Modal open onClose={onClose} maxWidth={480}>
      ...
      <div style={{ padding: '12px 16px 0', overflowY: 'auto', flex: 1 }}>
        <div style={{ background: hex(C.info, 0.07), border: `1px solid ${hex(C.info, 0.2)}`, borderRadius: 8, padding: 10, marginBottom: 12, display: 'flex', gap: 8 }}>
          <Info size={14} color={C.info} style={{ flexShrink: 0, marginTop: 1 }} />
          <p style={{ fontSize: 11, color: C.info }}>प्रत्येक कमरा एक मतदान स्थल है।</p>
        </div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <input value={roomNum} onChange={e => setRoomNum(e.target.value)} onKeyDown={e => e.key === 'Enter' && addRoom()} placeholder="कमरा नंबर / Room Number"
            style={{ flex: 1, padding: '10px 12px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', fontFamily: 'inherit' }} />
          <button onClick={addRoom} disabled={adding} style={{ padding: '10px 14px', background: C.primary, color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {adding ? <Spinner size={16} /> : <Plus size={18} />}
          </button>
        </div>
        {loading ? (
          ...
          <div key={room.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'white', border: `1px solid ${hex(C.border, 0.4)}`, borderRadius: 10, marginBottom: 8 }}>
            <div style={{ width: 32, height: 32, background: hex(C.primary, 0.1), borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: C.primary }}>{i + 1}</div>
            <DoorOpen size={15} color={C.subtle} />
            <p style={{ flex: 1, fontWeight: 600, fontSize: 13, color: C.dark }}>कमरा: {room.roomNumber}</p>
            <button onClick={() => deleteRoom(room.id)} style={{ width: 30, height: 30, background: hex(C.error, 0.08), border: `1px solid ${hex(C.error, 0.25)}`, borderRadius: 7, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Trash2 size={14} color={C.error} />
            </button>
          </div>
        ))}
      ...
      <div style={{ padding: '8px 16px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={onClose} style={{ padding: '9px 20px', border: `1px solid ${C.border}`, borderRadius: 10, background: 'transparent', color: C.subtle, cursor: 'pointer', fontSize: 13 }}>बंद करें</button>
      </div>
    </Modal>
  );
}
...
// ══════════════════════════════════════════════════════════════════════════════
//  SWAP STAFF SHEET — view assigned staff for a center and swap with reserve
// ══════════════════════════════════════════════════════════════════════════════
function SwapStaffSheet({ center, onSwapped, onClose }) {
  const [assigned, setAssigned] = useState([]);
  const [loading, setLoading]   = useState(true);
  const [swapping, setSwapping] = useState(false);
  const [pickerRank, setPickerRank] = useState(null); // rank being swapped
  const [swapTarget, setSwapTarget] = useState(null); // staff being removed
  ...
  const loadAssigned = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get(`/admin/center/${center.id}/staff`);
      const data = res.data;
  ...
  useEffect(() => { loadAssigned(); }, [center.id]);

  const handleSwap = async (removeStaff, newStaff) => {
    setSwapping(true);
    try {
      await apiClient.post('/admin/swap', {
        removeStaffId: removeStaff.id,
        addStaffId: newStaff.id,
        centerId: center.id,
      });
      toast.success('Swap सफल!');
      onSwapped();
      onClose();
    } catch (e) { toast.error(`Swap विफल: ${e.message}`); }
    finally { setSwapping(false); setPickerRank(null); setSwapTarget(null); }
  };

  return (
    <>
      <Modal open onClose={onClose} maxWidth={480} maxHeight="80vh">
        <ModalHeader title="Swap / Remove Staff" subtitle={center.name} Icon={ArrowDownUp} color={tc} onClose={onClose} />
        <div style={{ padding: '10px 16px 0' }}>
          <div style={{ background: hex(C.info, 0.07), border: `1px solid ${hex(C.info, 0.2)}`, borderRadius: 8, padding: 10, marginBottom: 12, display: 'flex', gap: 8 }}>
            <Info size={14} color={C.info} style={{ flexShrink: 0 }} />
            <p style={{ fontSize: 11, color: C.info }}>Swap: किसी को हटाएं और उसकी जगह Reserve से नया लगाएं</p>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 20px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: 40 }}>
              <User size={48} color={hex(C.subtle, 0.4)} />
              <p style={{ color: C.subtle, fontSize: 13, marginTop: 12 }}>कोई staff assign नहीं है</p>
            </div>
          ) : assigned.map(s => {
            const rank = s.user_rank || s.rank || '';
            const rc   = RANK_COLORS[rank] || C.primary;
            return (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'white', border: `1px solid ${hex(C.border, 0.4)}`, borderRadius: 10, marginBottom: 8 }}>
                <div style={{ width: 38, height: 38, borderRadius: '50%', background: hex(rc, 0.12), border: `1px solid ${hex(rc, 0.3)}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: rc, flexShrink: 0 }}>
                  {initials(s.name)}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 700, fontSize: 13, color: C.dark }}>{s.name}</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ padding: '1px 6px', background: hex(rc, 0.1), borderRadius: 5, fontSize: 9, fontWeight: 700, color: rc, border: `1px solid ${hex(rc, 0.3)}` }}>{rank}</span>
                    {s.mobile && <span style={{ fontSize: 10, color: C.subtle }}>{s.mobile}</span>}
                  </div>
                </div>
                <button onClick={() => { if (!swapping) { setSwapTarget(s); setPickerRank(rank); } }} disabled={swapping} style={{ padding: '6px 10px', background: hex(C.info, 0.08), border: `1px solid ${hex(C.info, 0.3)}`, borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, opacity: swapping ? 0.5 : 1 }}>
                  <ArrowDownUp size={13} color={C.info} />
                  <span style={{ fontSize: 11, fontWeight: 700, color: C.info }}>Swap</span>
                </button>
              </div>
            );
          })}
      ...
      {pickerRank && swapTarget && (
        <StaffPickerSheet
          allowedRanks={pickerRank ? [pickerRank, ...RANK_HIERARCHY.slice(RANK_HIERARCHY.indexOf(pickerRank) + 1).slice(0, 2)] : RANK_HIERARCHY}
          color={C.info}
          onPick={newStaff => handleSwap(swapTarget, newStaff)}
          onClose={() => { setPickerRank(null); setSwapTarget(null); }}
        />
      )}
    </>
  );
...
// ══════════════════════════════════════════════════════════════════════════════
//  ASSIGN DUTY CONFIRM DIALOG
// ══════════════════════════════════════════════════════════════════════════════
function AssignDutyConfirmDialog({ szName, color, onConfirm, onClose }) {
  return (
    <Modal open onClose={onClose} maxWidth={400}>
      <ModalHeader title="Duty Assignment" Icon={ClipboardList} color={color} onClose={onClose} />
      <div style={{ padding: 16 }}>
        <p style={{ color: C.dark, fontSize: 13, marginBottom: 10 }}>"{szName}" के सभी centers पर मानक के अनुसार स्टाफ असाइन होगा।</p>
        <div style={{ padding: 9, background: hex(C.orange, 0.08), borderRadius: 8, border: `1px solid ${hex(C.orange, 0.3)}`, display: 'flex', gap: 6, marginBottom: 16 }}>
          <Info size={13} color={C.orange} style={{ flexShrink: 0 }} />
          <p style={{ fontSize: 11, color: C.orange }}>यह background में चलेगा। आप बाकी काम जारी रख सकते हैं।</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px 0', border: `1px solid ${C.border}`, borderRadius: 10, background: 'transparent', color: C.subtle, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>रद्द</button>
          <button onClick={onConfirm} style={{ flex: 1, padding: '11px 0', border: 'none', borderRadius: 10, background: color, color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>Start</button>
        </div>
      </div>
    </Modal>
  );
...
// ══════════════════════════════════════════════════════════════════════════════
function UnlockRequestDialog({ szName, onConfirm, onClose }) {
  const [reason, setReason] = useState('');
  return (
    <Modal open onClose={onClose} maxWidth={380}>
      <ModalHeader title="Unlock Request" Icon={Unlock} color={C.info} onClose={onClose} />
      <div style={{ padding: 16 }}>
        <p style={{ color: C.dark, fontSize: 13, marginBottom: 8 }}>Unlock का कारण दर्ज करें:</p>
        <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="कारण लिखें..."
          rows={3} style={{ width: '100%', padding: '9px 12px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', resize: 'vertical', fontFamily: 'inherit', marginBottom: 14 }} />
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px 0', border: `1px solid ${C.border}`, borderRadius: 10, background: 'transparent', color: C.subtle, cursor: 'pointer', fontSize: 13 }}>रद्द</button>
          <button onClick={() => onConfirm(reason)} style={{ flex: 1, padding: '11px 0', border: 'none', borderRadius: 10, background: C.info, color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>Request भेजें</button>
        </div>
      </div>
    </Modal>
  );
...
//  LOCK CONFIRM DIALOG
// ══════════════════════════════════════════════════════════════════════════════
function LockConfirmDialog({ onConfirm, onClose }) {
  const [reason, setReason] = useState('');
  return (
    <Modal open onClose={onClose} maxWidth={380}>
      <ModalHeader title="Lock Duties" Icon={Lock} color={C.success} onClose={onClose} />
      <div style={{ padding: 16 }}>
        <p style={{ color: C.dark, fontSize: 13, marginBottom: 8 }}>Lock करने के बाद manual changes बंद हो जाएंगे।</p>
        <input value={reason} onChange={e => setReason(e.target.value)} placeholder="कारण (Optional)"
          style={{ width: '100%', padding: '10px 12px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', fontFamily: 'inherit', marginBottom: 14 }} />
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px 0', border: `1px solid ${C.border}`, borderRadius: 10, background: 'transparent', color: C.subtle, cursor: 'pointer', fontSize: 13 }}>रद्द</button>
          <button onClick={() => onConfirm(reason)} style={{ flex: 1, padding: '11px 0', border: 'none', borderRadius: 10, background: C.success, color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>Lock करें</button>
        </div>
      </div>
    </Modal>
  );
...
// ══════════════════════════════════════════════════════════════════════════════
//  SUPER ZONE ITEM CARD (enhanced with Assign / Refresh / Lock)
// ══════════════════════════════════════════════════════════════════════════════
function SuperZoneItemCard({ item, color, isSelected, onTap, onEdit, onDelete, onAssign, onRefresh, onLock, jobRunning }) {
  const officers    = item.officers || [];
  const isLocked    = (item.is_locked ?? 0) === 1;
  const centerCount = item.center_count ?? item.centerCount ?? 0;

  return (
    <div onClick={onTap} style={{
      marginBottom: 8, padding: '10px 8px 10px 12px',
      background: isSelected ? hex(color, 0.07) : 'white',
      border: `${isSelected || jobRunning ? 2 : 1}px solid ${jobRunning ? C.orange : isLocked ? hex(C.success, 0.5) : isSelected ? color : hex(C.border, 0.4)}`,
      borderRadius: 12, cursor: 'pointer', transition: 'all 0.15s',
      boxShadow: `0 2px 6px ${hex(color, 0.05)}`,
    }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        {/* Avatar */}
        <div style={{ width: 38, height: 38, borderRadius: '50%', flexShrink: 0, background: jobRunning ? hex(C.orange, 0.15) : hex(color, 0.12), border: `1px solid ${jobRunning ? hex(C.orange, 0.4) : hex(color, 0.3)}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {jobRunning ? <Spinner size={16} color={C.orange} /> : isSelected ? <Check size={18} color={color} /> : <Layers size={18} color={color} />}
        </div>

        {/* Info */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, flexWrap: 'wrap' }}>
            <p style={{ fontWeight: 700, fontSize: 14, color: isSelected ? color : C.dark, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{item.name}</p>
            {isLocked && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 6px', background: hex(C.success, 0.1), border: `1px solid ${hex(C.success, 0.4)}`, borderRadius: 5, fontSize: 9, fontWeight: 700, color: C.success, flexShrink: 0 }}>
                <Lock size={9} /> Locked
              </span>
            )}
            {isSelected && !isLocked && (
              <span style={{ padding: '2px 7px', background: hex(color, 0.12), border: `1px solid ${hex(color, 0.3)}`, borderRadius: 6, fontSize: 10, fontWeight: 700, color, flexShrink: 0 }}>चुना गया</span>
            )}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 10px' }}>
            ...
            {item.sectorCount != null && <Tag icon={LayoutGrid} text={`${item.sectorCount} Sectors`} />}
            {item.gpCount     != null && <Tag icon={Landmark}   text={`${item.gpCount} GPs`} />}
            {centerCount > 0          && <Tag icon={MapPin}     text={`${centerCount} Centers`} />}
          </div>
          {officers.slice(0, 3).length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 5px', marginTop: 6 }}>
              {officers.slice(0, 3).map((o, i) => (
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 10 }} onClick={e => e.stopPropagation()}>
        {/* Assign Duty */}
        <ActionBtn
          icon={jobRunning ? RefreshCw : ClipboardList}
          label={jobRunning ? 'Running...' : 'Assign Duty'}
          color={C.orange}
          disabled={jobRunning || isLocked}
          spinning={jobRunning}
          onClick={onAssign}
        />
        {/* Refresh */}
        ...
        <ActionBtn
          ...
          color={isLocked ? C.error : C.success}
          onClick={onLock}
        />
      </div>
    </div>
...
function ActionBtn({ icon: Icon, label, color, disabled = false, spinning = false, onClick }) {
  return (
    <button onClick={disabled ? undefined : onClick} style={{
      flex: 1, padding: '7px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4,
      background: disabled ? hex('#888', 0.06) : hex(color, 0.08),
      border: `1px solid ${disabled ? hex('#888', 0.2) : hex(color, 0.35)}`,
      borderRadius: 8, cursor: disabled ? 'default' : 'pointer',
      transition: 'all 0.12s',
    }}>
      {spinning ? <Spinner size={13} color={color} /> : <Icon size={13} color={disabled ? '#aaa' : color} />}
      <span style={{ fontSize: 10, fontWeight: 700, color: disabled ? '#aaa' : color, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</span>
    </button>
  );
}

// ══════════════════════════════════════════════════════════════════════════════
//  ITEM CARD (generic, for Zone / Sector / GP)
// ══════════════════════════════════════════════════════════════════════════════
function ItemCard({ item, color, Icon: I, isSelected, onTap, onEdit, onDelete }) {
  const officers = item.officers || [];
  return (
    <div onClick={onTap} style={{
      ...
      <div style={{ width: 38, height: 38, borderRadius: '50%', flexShrink: 0, background: hex(color, 0.12), border: `1px solid ${hex(color, 0.3)}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {isSelected ? <Check size={18} color={color} /> : <I size={18} color={color} />}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
...
//  CENTER CARD
// ══════════════════════════════════════════════════════════════════════════════
function CenterCard({ center, onEdit, onDelete, onSwap }) {
  const type       = center.centerType || 'C';
  const tc         = TYPE_COLORS[type] || C.info;
  const assigned   = center.assignedStaff || [];
  const missing    = center.missingRanks  || [];
  const dutyCount  = center.dutyCount ?? assigned.length;
  const roomCount  = center.roomCount ?? 0;
  const boothCount = center.boothCount ?? center.booth_count ?? 0;
  const [showRooms, setShowRooms] = useState(false);
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 8px 10px 12px' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 8px' }}>
              {center.thana  && <Tag icon={ShieldCheck}   text={center.thana} />}
              {center.busNo  && <Tag icon={Badge}         text={`Bus: ${center.busNo}`} />}
              {boothCount > 0 && <Tag icon={CheckCircle2} text={`${boothCount} बूथ`} />}
              <Tag icon={User}   text={`${dutyCount} स्टाफ`} />
              {roomCount > 0 && <Tag icon={DoorOpen} text={`${roomCount} कमरे`} />}
            </div>
          </div>
          ...
          <div style={{ padding: '0 12px 10px', display: 'flex', flexWrap: 'wrap', gap: '5px 6px' }}>
            {assigned.slice(0, 5).map((s, i) => {
              ...
              return (
                <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 7px', background: hex(C.success, 0.06), border: `1px solid ${hex(C.success, 0.25)}`, borderRadius: 7 }}>
                  <CheckCircle2 size={11} color={C.success} />
                  <span style={{ fontSize: 11, fontWeight: 600, color: C.dark, maxWidth: 70, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</span>
                  <span style={{ padding: '1px 4px', background: hex(rc, 0.12), borderRadius: 4, fontSize: 9, fontWeight: 700, color: rc }}>{s.rank}</span>
                </span>
              );
            })}
            {assigned.length > 5 && <span style={{ padding: '4px 7px', background: hex(C.subtle, 0.08), borderRadius: 7, fontSize: 11, color: C.subtle }}>+{assigned.length - 5} और</span>}
          </div>
        )}

        {/* Action bar */}
        <div style={{ display: 'flex', gap: 8, margin: '0 12px 10px' }}>
          <button onClick={onSwap} style={{ flex: 1, padding: '7px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, background: hex(C.info, 0.07), border: `1px solid ${hex(C.info, 0.25)}`, borderRadius: 8, cursor: 'pointer' }}>
            <ArrowDownUp size={13} color={C.info} />
            <span style={{ fontSize: 11, fontWeight: 700, color: C.info }}>Swap Staff</span>
          </button>
          <button onClick={() => setShowRooms(true)} style={{ flex: 1, padding: '7px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, background: hex(C.primary, 0.07), border: `1px solid ${hex(C.primary, 0.25)}`, borderRadius: 8, cursor: 'pointer' }}>
            <DoorOpen size={13} color={C.primary} />
            <span style={{ fontSize: 11, fontWeight: 700, color: C.primary }}>Rooms</span>
          </button>
        </div>
      </div>

      {showRooms && <MatdanSthalDialog centerId={center.id} centerName={center.name} onClose={() => setShowRooms(false)} />}
    </>
  );
}

// ══════════════════════════════════════════════════════════════════════════════
//  CENTER DIALOG — Add/Edit with boothCount field
// ══════════════════════════════════════════════════════════════════════════════
function CenterDialog({ gpId, existing, onDone, onClose }) {
  const [form, setForm] = useState({
    name:       existing?.name        || '',
    ...
    longitude:  existing?.longitude?.toString() || '',
    centerType: existing?.centerType  || 'C',
    boothCount: (existing?.boothCount ?? existing?.booth_count ?? 1).toString(),
  });
  const prevType = useRef(existing?.centerType || 'C');
  const [saving, setSaving]           = useState(false);
  const [showRooms, setShowRooms]     = useState(false);
  ...
  const incrementBooth = (delta) => {
    const cur = parseInt(form.boothCount) || 1;
    const next = Math.max(1, Math.min(15, cur + delta));
    setField('boothCount', String(next));
  };

  const save = async () => {
    if (!form.name.trim()) { toast.error('नाम आवश्यक है'); return; }
    const boothCount = parseInt(form.boothCount) || 1;
    if (boothCount < 1) { toast.error('बूथ संख्या कम से कम 1 होनी चाहिए'); return; }
    setSaving(true);
    try {
      const body = {
        name: form.name.trim(), address: form.address.trim(), thana: form.thana.trim(),
        busNo: form.busNo.trim(), centerType: form.centerType, boothCount,
        latitude:  form.latitude  ? parseFloat(form.latitude)  : null,
        longitude: form.longitude ? parseFloat(form.longitude) : null,
      };
      if (existing) { await apiClient.put(`/admin/centers/${existing.id}`, body); }
      else           { await apiClient.post(`/admin/gram-panchayats/${gpId}/centers`, body); }
      onDone(); onClose();
    } catch (e) { toast.error(`Error: ${e.message}`); }
    finally { setSaving(false); }
  };
  ...
  return (
    <>
      <Modal open onClose={onClose} maxWidth={520}>
        <ModalHeader title={existing ? 'Center संपादित करें' : 'Election Center जोड़ें'} Icon={MapPin} color={color} onClose={onClose} />
        <div style={{ overflowY: 'auto', flex: 1, padding: 16 }}>
          <FieldInput label="Center का नाम *" icon={MapPin}     color={color} value={form.name}    onChange={v => setField('name', v)} />
          <FieldInput label="पता"              icon={MapPin}     color={color} value={form.address} onChange={v => setField('address', v)} />
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1 }}><FieldInput label="थाना"   icon={ShieldCheck} color={color} value={form.thana} onChange={v => setField('thana', v)} /></div>
            <div style={{ flex: 1 }}><FieldInput label="Bus No" icon={Badge}       color={color} value={form.busNo} onChange={v => setField('busNo', v)} /></div>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1 }}><FieldInput label="Latitude"  icon={MapPin} color={color} value={form.latitude}  onChange={v => setField('latitude', v)} /></div>
            <div style={{ flex: 1 }}><FieldInput label="Longitude" icon={MapPin} color={color} value={form.longitude} onChange={v => setField('longitude', v)} /></div>
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <div style={{ width: 3, height: 14, background: color, borderRadius: 2 }} />
              <p style={{ color: C.dark, fontSize: 13, fontWeight: 800 }}>बूथ संख्या *</p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button onClick={() => incrementBooth(-1)} style={{ width: 48, height: 48, background: hex(color, 0.1), border: `1px solid ${hex(color, 0.4)}`, borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: 22, color, fontWeight: 700 }}>−</span>
              </button>
              <div style={{ flex: 1, height: 48, background: 'white', border: `1.5px solid ${hex(color, 0.6)}`, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <input type="number" value={form.boothCount} min={1} max={15}
                  onChange={e => { const v = parseInt(e.target.value); if (v >= 1 && v <= 15) setField('boothCount', String(v)); }}
                  style={{ width: '100%', textAlign: 'center', fontSize: 20, fontWeight: 900, color, background: 'transparent', border: 'none', outline: 'none', fontFamily: 'inherit' }} />
              </div>
              <button onClick={() => incrementBooth(1)} style={{ width: 48, height: 48, background: hex(color, 0.1), border: `1px solid ${hex(color, 0.4)}`, borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: 22, color, fontWeight: 700 }}>+</span>
              </button>
            </div>
            <p style={{ textAlign: 'center', fontSize: 10, color: hex(C.subtle, 0.7), marginTop: 4 }}>(1 से 15 तक — 15 = 15 और उससे अधिक)</p>
          </div>

          {/* Rooms shortcut (edit only) */}
          {existing && (
            <button onClick={() => setShowRooms(true)} style={{ width: '100%', padding: 12, background: hex(C.info, 0.06), border: `1px solid ${hex(C.info, 0.25)}`, borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, textAlign: 'left' }}>
              <DoorOpen size={16} color={C.info} />
              <div style={{ flex: 1 }}>
                <p style={{ color: C.info, fontWeight: 700, fontSize: 13 }}>मतदान स्थल (Matdan Sthal) / कमरे</p>
                <p style={{ color: C.subtle, fontSize: 11 }}>{existing.roomCount || 0} कमरे — प्रबंधन के लिए टैप करें</p>
              </div>
              <ChevronRight size={14} color={C.info} />
            </button>
          )}
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            {CENTER_TYPES.map(t => {
              ...
              const sel = form.centerType === t;
              return (
                <button key={t} onClick={() => setField('centerType', t)} style={{ flex: 1, padding: '10px 0', border: `${sel ? 2 : 1}px solid ${tc}`, borderRadius: 10, cursor: 'pointer', background: sel ? tc : 'white', transition: 'all 0.15s' }}>
                  <p style={{ color: sel ? 'white' : tc, fontWeight: 900, fontSize: 14 }}>{t}</p>
                  <p style={{ color: sel ? 'rgba(255,255,255,0.7)' : hex(tc, 0.7), fontSize: 9 }}>{TYPE_LABELS[t]}</p>
                </button>
              );
          ...
          <div style={{ padding: 10, background: hex(C.info, 0.06), border: `1px solid ${hex(C.info, 0.2)}`, borderRadius: 8, display: 'flex', gap: 8 }}>
            <Info size={14} color={C.info} style={{ flexShrink: 0 }} />
            <p style={{ fontSize: 11, color: C.info }}>Center बनने के बाद Duty Assignment Super Zone level से "Assign Duty" बटन द्वारा होगी। बूथ संख्या के अनुसार मानक (booth_rules) लागू होगा।</p>
          </div>
        </div>
        <div style={{ padding: '8px 16px 16px', display: 'flex', gap: 12 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '12px 0', border: `1px solid ${C.border}`, borderRadius: 10, background: 'transparent', color: C.subtle, cursor: 'pointer', fontSize: 13 }}>रद्द</button>
          <button onClick={save} disabled={saving} style={{ flex: 1, padding: '12px 0', border: 'none', borderRadius: 10, background: color, color: 'white', cursor: 'pointer', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: saving ? 0.7 : 1 }}>
            {saving ? <Spinner /> : (existing ? 'अपडेट करें' : 'Center जोड़ें')}
          </button>
        </div>
      </Modal>
      {showRooms && <MatdanSthalDialog centerId={existing?.id} centerName={form.name} onClose={() => setShowRooms(false)} />}
    </>
  );
}

// ══════════════════════════════════════════════════════════════════════════════
//  SUPER ZONE LIST (step 0) — with Assign / Refresh / Lock background job logic
// ══════════════════════════════════════════════════════════════════════════════
function SuperZoneList({ onSelect, selectedId }) {
  const color = '#6A1B9A';
  const jobs  = useJobStore();
  ...
  const load = useCallback(async (reset = false) => {
    if (!reset && !hasMoreRef.current) return;
    if (reset) { pageRef.current = 1; hasMoreRef.current = true; setItems([]); setLoading(true); }
    else setLoadingMore(true);
    try {
      const res = await apiClient.get('/admin/super-zones', { params: { page: pageRef.current, limit: LIMIT, q: qRef.current } });
      let fetched = [], totalPages = 1;
      if (Array.isArray(res.data))      { fetched = res.data; }
      else if (res.data?.data)          { fetched = res.data.data; totalPages = res.data.totalPages || 1; }
      hasMoreRef.current = pageRef.current < totalPages;
  ...
  useEffect(() => { qRef.current = ''; load(true); }, []);
  ...
  const handleDelete = async () => {
    try { await apiClient.delete(`/admin/super-zones/${deleteTarget.id}`); toast.success('Deleted'); setDeleteTarget(null); load(true); }
    catch (e) { toast.error(`Delete failed: ${e.message}`); }
  };

  // ── Assign Duty ────────────────────────────────────────────────────────────
  const startAssignJob = async (szItem) => {
    const szId   = szItem.id;
    const szName = szItem.name || '';
    setAssignTarget(null);
    try {
      const res  = await apiClient.post(`/admin/assign/start/${szId}`, {});
      const jobId = res.data?.data?.jobId || res.data?.jobId || 0;
      if (!jobId) { toast.error('Job शुरू नहीं हुआ'); return; }
      activeJobs[szId] = { szId, szName, jobId, status: 'running' };
      notifyJobChange();
      pollJobStatus(szId, jobId);
      toast.success(`${szName} assignment शुरू हो गई (background में)`);
    } catch (e) { toast.error(`Assignment शुरू नहीं हुई: ${e.message}`); }
  };

  const pollJobStatus = (szId, jobId) => {
    const interval = setInterval(async () => {
      try {
        const res    = await apiClient.get(`/admin/assign/status/${jobId}`);
        const job    = res.data?.data || res.data || {};
        const status = job.status || 'pending';

        if (activeJobs[szId]) { activeJobs[szId].status = status; notifyJobChange(); }

        if (status === 'done' || status === 'error') {
          clearInterval(interval);
          if (status === 'done') {
            const szName = activeJobs[szId]?.szName || '';
            delete activeJobs[szId];
            notifyJobChange();
            load(true);
            toast.success(`${szName} Assignment पूर्ण!`);
          } else {
            toast.error(`Assignment विफल: ${job.error_msg || 'Unknown error'}`);
            delete activeJobs[szId];
            notifyJobChange();
          }
        }
  ...
  // ── Refresh Duties (unassign all) ─────────────────────────────────────────
  const doRefresh = async (szItem) => {
    setRefreshTarget(null);
    try {
      await apiClient.post(`/admin/refresh/${szItem.id}`, {});
      toast.success('सभी assignments हटाई गईं');
      load(true);
    } catch (e) { toast.error(`Error: ${e.message}`); }
  };

  // ── Lock ──────────────────────────────────────────────────────────────────
  const doLock = async (reason) => {
    const szItem = lockTarget;
    setLockTarget(null);
    try {
      await apiClient.post(`/admin/lock/${szItem.id}`, { reason: reason.trim() });
  ...
  // ── Unlock request ─────────────────────────────────────────────────────────
  const doUnlockRequest = async (reason) => {
    const szItem = unlockTarget;
    setUnlockTarget(null);
    try {
      await apiClient.post('/admin/unlock/request', { superZoneId: szItem.id, reason: reason.trim() });
      toast.success('Unlock request भेजी गई');
      load(true);
    } catch (e) { toast.error(`Error: ${e.message}`); }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Header */}
      <div style={{ background: C.surface, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <div style={{ padding: 7, background: hex(color, 0.12), borderRadius: 8 }}><Layers size={16} color={color} /></div>
        <p style={{ flex: 1, color: C.dark, fontWeight: 800, fontSize: 15 }}>Super Zones</p>
        <button onClick={() => setDialog('add')} style={{ padding: '7px 12px', background: color, color: 'white', border: 'none', borderRadius: 9, cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
          <Plus size={14} /> जोड़ें
        </button>
      </div>
      {/* Search */}
      <div style={{ background: C.bg, padding: '8px 12px', flexShrink: 0 }}>
        <div style={{ position: 'relative' }}>
          <Search size={16} color={C.subtle} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)' }} />
          <input placeholder="Super Zones खोजें..." onChange={e => { setQ(e.target.value); debouncedSearch(e.target.value); }}
            style={{ width: '100%', padding: '9px 32px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          {q && <button onClick={() => { setQ(''); qRef.current = ''; load(true); }} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer' }}><X size={14} color={C.subtle} /></button>}
        </div>
      </div>
      ...
      <div ref={scrollRef} onScroll={handleScroll} style={{ flex: 1, overflowY: 'auto', padding: '8px 12px 80px' }}>
        {loading ? (
          ...
          <EmptyState label="Super Zones" Icon={Layers} color={color} />
        ) : items.map(item => {
          const szId      = item.id;
          const job       = jobs[szId];
          ...
          const isLocked  = (item.is_locked ?? 0) === 1;
          return (
            <SuperZoneItemCard key={szId}
              item={item} color={color} isSelected={selectedId === szId}
              jobRunning={jobRunning}
              onTap={() => onSelect(item)}
              onEdit={() => setDialog(item)}
              onDelete={() => setDeleteTarget(item)}
              onAssign={() => {
                if (job && jobRunning) { toast.error(`${item.name} के लिए assignment पहले से चल रही है`); return; }
                setAssignTarget(item);
              }}
              onRefresh={() => setRefreshTarget(item)}
              onLock={() => isLocked ? setUnlockTarget(item) : setLockTarget(item)}
            />
          );
        })}
        {loadingMore && <div style={{ display: 'flex', justifyContent: 'center', padding: 12 }}><Spinner color={C.primary} size={16} /></div>}
      ...
      {dialog && (
        <ItemDialog
          title={dialog === 'add' ? 'Super Zone जोड़ें' : 'संपादित करें'}
          color={color} Icon={Layers}
          fields={['name', 'district', 'block']}
          officerTitle={LEVEL_OFFICER_TITLE[0]} officerRanks={LEVEL_RANKS[0]}
          existing={dialog === 'add' ? null : dialog}
          createUrl="/admin/super-zones"
          updateUrlFn={id => `/admin/super-zones/${id}`}
          onDone={() => load(true)} onClose={() => setDialog(null)} />
      )}
      <ConfirmDialog open={!!deleteTarget} message={`Delete "${deleteTarget?.name}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />

      {/* Assign confirm */}
      {assignTarget && (
        <AssignDutyConfirmDialog szName={assignTarget.name} color={color}
          onConfirm={() => startAssignJob(assignTarget)} onClose={() => setAssignTarget(null)} />
      )}
      ...
      <ConfirmDialog
        ...
        message={`"${refreshTarget?.name}" के सभी assignments हट जाएंगे। स्टाफ Reserve में आ जाएगा।`}
        confirmColor={C.error} confirmLabel="हटाएं"
        Icon={RefreshCw}
        onConfirm={() => doRefresh(refreshTarget)}
        onCancel={() => setRefreshTarget(null)}
      />

      {/* Lock confirm */}
      {lockTarget && <LockConfirmDialog onConfirm={doLock} onClose={() => setLockTarget(null)} />}

      {/* Unlock request */}
      {unlockTarget && <UnlockRequestDialog szName={unlockTarget.name} onConfirm={doUnlockRequest} onClose={() => setUnlockTarget(null)} />}
    </div>
  );
}

// ══════════════════════════════════════════════════════════════════════════════
//  GENERIC STEP LIST (Zone / Sector / GP)
// ══════════════════════════════════════════════════════════════════════════════
function StepList({ title, Icon: I, color, officerTitle, officerRanks, fetchUrl, createUrl, updateUrlFn, deleteUrlFn, fields, onSelect, selectedId }) {
  const [items, setItems]         = useState([]);
  const [loading, setLoading]     = useState(true);
  ...
  const load = useCallback(async (reset = false) => {
    ...
    try {
      const res = await apiClient.get(fetchUrl, { params: { page: pageRef.current, limit: LIMIT, q: qRef.current } });
      let fetched = [], totalPages = 1;
      if (Array.isArray(res.data))      { fetched = res.data; }
      else if (res.data?.data)          { fetched = res.data.data; totalPages = res.data.totalPages || 1; }
      else if (Array.isArray(res))      { fetched = res; }
  ...
  const debouncedSearch = useCallback(debounce((v) => { qRef.current = v; load(true); }, 350), [load]);
  const handleScroll = () => { const el = scrollRef.current; if (!el) return; if (el.scrollTop + el.clientHeight >= el.scrollHeight - 200 && !loadingMore) load(); };

  const handleDelete = async () => {
    try { await apiClient.delete(deleteUrlFn(deleteTarget.id)); toast.success('Deleted'); setDeleteTarget(null); load(true); }
    catch (e) { toast.error(`Delete failed: ${e.message}`); }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ background: C.surface, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <div style={{ padding: 7, background: hex(color, 0.12), borderRadius: 8 }}><I size={16} color={color} /></div>
        <p style={{ flex: 1, color: C.dark, fontWeight: 800, fontSize: 15 }}>{title}</p>
        <button onClick={() => setDialog('add')} style={{ padding: '7px 12px', background: color, color: 'white', border: 'none', borderRadius: 9, cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
          <Plus size={14} /> जोड़ें
        </button>
      ...
      <div style={{ background: C.bg, padding: '8px 12px', flexShrink: 0 }}>
        <div style={{ position: 'relative' }}>
          <Search size={16} color={C.subtle} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)' }} />
          <input placeholder={`${title} खोजें...`} onChange={e => { setQ(e.target.value); debouncedSearch(e.target.value); }}
            style={{ width: '100%', padding: '9px 32px', background: 'white', border: `1.2px solid ${C.border}`, borderRadius: 10, fontSize: 13, color: C.dark, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          {q && <button onClick={() => { setQ(''); qRef.current = ''; load(true); }} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer' }}><X size={14} color={C.subtle} /></button>}
        </div>
      </div>
      ...
      {dialog && (
        <ItemDialog
          title={dialog === 'add' ? `${title.replace(/s$/, '')} जोड़ें` : 'संपादित करें'}
          color={color} Icon={I} fields={fields}
          officerTitle={officerTitle} officerRanks={officerRanks}
          existing={dialog === 'add' ? null : dialog}
          createUrl={createUrl} updateUrlFn={updateUrlFn}
          onDone={() => load(true)} onClose={() => setDialog(null)} />
      )}
      <ConfirmDialog open={!!deleteTarget} message={`Delete "${deleteTarget?.name}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
    </div>
...
function CenterStep({ gpId }) {
  ...
  const [dialog, setDialog]       = useState(null);
  const [deleteTarget, setDeleteTarget] = useState(null);
  const [swapCenter, setSwapCenter] = useState(null);
  const scrollRef  = useRef(null);
  const pageRef    = useRef(1);
  const hasMoreRef = useRef(true);
  const qRef       = useRef('');
  ...
  const load = useCallback(async (reset = false) => {
    ...
    try {
      const res = await apiClient.get(`/admin/gram-panchayats/${gpId}/centers`, { params: { page: pageRef.current, limit: 20, q: qRef.current } });
      let items = [], totalPages = 1;
      if (Array.isArray(res.data))      { items = res.data; }
      else if (res.data?.data)          { items = res.data.data; totalPages = res.data.totalPages || 1; }
      else if (Array.isArray(res))      { items = res; }
  ...
  const debouncedSearch = useCallback(debounce((v) => { qRef.current = v; load(true); }, 350), [load]);
  const handleScroll = () => { const el = scrollRef.current; if (!el) return; if (el.scrollTop + el.clientHeight >= el.scrollHeight - 200) load(); };

  const handleDelete = async () => {
    try { await apiClient.delete(`/admin/centers/${deleteTarget.id}`); toast.success('Deleted'); setDeleteTarget(null); load(true); }
    catch (e) { toast.error(`Delete failed: ${e.message}`); }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ background: C.surface, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <div style={{ padding: 7, background: hex('#C62828', 0.12), borderRadius: 8 }}><MapPin size={16} color="#C62828" /></div>
        <p style={{ flex: 1, color: C.dark, fontWeight: 800, fontSize: 15 }}>Election Centers</p>
        <button onClick={() => setDialog('add')} style={{ padding: '7px 12px', background: '#C62828', color: 'white', border: 'none', borderRadius: 9, cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
          <Plus size={14} /> जोड़ें
        </button>
      ...
      <div ref={scrollRef} onScroll={handleScroll} style={{ flex: 1, overflowY: 'auto', padding: '8px 12px 80px' }}>
        {loading ? (
          ...
          <CenterCard key={c.id} center={c}
            onEdit={() => setDialog(c)}
            onDelete={() => setDeleteTarget(c)}
            onSwap={() => setSwapCenter(c)} />
        ))}
        {loadingMore && <div style={{ display: 'flex', justifyContent: 'center', padding: 12 }}><Spinner color={C.primary} size={16} /></div>}
      </div>
      {dialog && <CenterDialog gpId={gpId} existing={dialog === 'add' ? null : dialog} onDone={() => load(true)} onClose={() => setDialog(null)} />}
      <ConfirmDialog open={!!deleteTarget} message={`Delete "${deleteTarget?.name}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
      {swapCenter && <SwapStaffSheet center={swapCenter} onSwapped={() => load(true)} onClose={() => setSwapCenter(null)} />}
    </div>
  );
}
  ...
  return (
    <div style={{ background: hex(C.surface, 0.7), padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0, overflowX: 'auto' }}>
      {crumbs.map((c, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
          {i > 0 && <ChevronRight size={14} color={C.subtle} />}
          <button onClick={() => onTap(c.step)} style={{ padding: '3px 8px', background: hex(c.color, i === crumbs.length - 1 ? 0.12 : 0.06), border: `1px solid ${hex(c.color, i === crumbs.length - 1 ? 0.4 : 0.2)}`, borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, maxWidth: 120 }}>
            <c.Icon size={11} color={c.color} />
            <span style={{ fontSize: 11, fontWeight: i === crumbs.length - 1 ? 700 : 500, color: c.color, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</span>
          </button>
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ background: C.dark, padding: '10px 8px', display: 'flex', gap: 4, flexShrink: 0 }}>
      {STEPS.map(({ id, label, Icon: I, color }) => {
        ...
        const isEn  = isEnabled(id);
        return (
          <button key={id} onClick={() => isEn && onTap(id)} disabled={!isEn} style={{ flex: 1, padding: '7px 0', background: isCur ? color : isDone ? hex(color, 0.2) : 'rgba(255,255,255,0.07)', border: `1px solid ${isCur ? color : isDone ? hex(color, 0.4) : 'rgba(255,255,255,0.15)'}`, borderRadius: 10, cursor: isEn ? 'pointer' : 'default', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            {isDone
              ? <Check size={16} color={isCur ? 'white' : color} />
              : <I size={16} color={isCur ? 'white' : isDone ? color : 'rgba(255,255,255,0.3)'} />
            }
            <span style={{ fontSize: 9, fontWeight: isCur ? 800 : 500, color: isCur ? 'white' : isDone ? color : 'rgba(255,255,255,0.35)' }}>{label}</span>
          </button>
        );
      })}
    </div>
...
export default function FormPage() {
  ...
  const onSZSelected     = (item) => { setSelected({ szId: item.id, szName: item.name, zoneId: null, zoneName: null, sectorId: null, sectorName: null, gpId: null, gpName: null }); setStep(1); };
  const onZoneSelected   = (item) => { setSelected(p => ({ ...p, zoneId: item.id, zoneName: item.name, sectorId: null, sectorName: null, gpId: null, gpName: null })); setStep(2); };
  const onSectorSelected = (item) => { setSelected(p => ({ ...p, sectorId: item.id, sectorName: item.name, gpId: null, gpName: null })); setStep(3); };
  const onGPSelected     = (item) => { setSelected(p => ({ ...p, gpId: item.id, gpName: item.name })); setStep(4); };

  const renderStep = () => {
    switch (step) {
      case 0: return <SuperZoneList key="sz" onSelect={onSZSelected} selectedId={selected.szId} />;
      case 1: return (
        <StepList key={`zone_${selected.szId}`} title="Zones" Icon={Grid} color="#1565C0"
          officerTitle={LEVEL_OFFICER_TITLE[1]} officerRanks={LEVEL_RANKS[1]}
          fetchUrl={`/admin/super-zones/${selected.szId}/zones`}
      ...
      case 3: return (
        <StepList key={`gp_${selected.sectorId}`} title="Gram Panchayats" Icon={Landmark} color="#6D4C41"
          ...
          fetchUrl={`/admin/sectors/${selected.sectorId}/gram-panchayats`}
          createUrl={`/admin/sectors/${selected.sectorId}/gram-panchayats`}
          updateUrlFn={id => `/admin/gram-panchayats/${id}`} deleteUrlFn={id => `/admin/gram-panchayats/${id}`}
          fields={['name', 'address']}
          onSelect={onGPSelected} selectedId={selected.gpId} />
      );
      case 4: return <CenterStep key={`center_${selected.gpId}`} gpId={selected.gpId} />;
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: C.bg, fontFamily: "'Tiro Devanagari Hindi', Georgia, serif" }}>
        ...
        <StepBar currentStep={step} onTap={goToStep} szName={selected.szName} zoneName={selected.zoneName} sectorName={selected.sectorName} gpName={selected.gpName} />

        {step > 0 && (
          <Breadcrumb step={step} szName={selected.szName} zoneName={selected.zoneName} sectorName={selected.sectorName} gpName={selected.gpName} onTap={goToStep} />
        )}
