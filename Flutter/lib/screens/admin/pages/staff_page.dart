...
import '../../../core/encoding_helper.dart';
...
class _StaffPageState extends State<StaffPage>
  ...
  void _onUploadChanged() {
    ...
    if (up.phase == _UploadPhase.done && _lastSeenPhase != _UploadPhase.done) {
      ...
      if (mounted)
        _snack(up.errorMsg.isNotEmpty ? up.errorMsg : 'अपलोड विफल', error: true);
  ...
  Future<void> _deleteStaff(Map s) async {
    final ok = await _confirm(
        'स्टाफ हटाएं', '"${_v(s['name'])}" को स्थायी रूप से हटाएं?', 'हटाएं');
    if (ok != true) return;
    try {
  ...
  Future<void> _removeDuty(Map s) async {
    ...
    final ok = await _confirm('ड्यूटी हटाएं',
        '"${_v(s['name'])}" को रिज़र्व में करें?', 'रिज़र्व करें');
    if (ok != true) return;
    try {
      final token = await AuthService.getToken();
      if (s['dutyId'] != null) {
  ...
  Future<void> _bulkDelete() async {
    ...
    final ok = await _confirm('$count स्टाफ हटाएं',
        '$count स्टाफ को स्थायी रूप से हटाएं?', 'हटाएं');
    if (ok != true) return;
    try {
      final token = await AuthService.getToken();
  ...
  Future<void> _bulkUnassign() async {
    ...
    final ok = await _confirm('${boothIds.length} ड्यूटी हटाएं',
        '${boothIds.length} बूथ स्टाफ रिज़र्व में जाएंगे।', 'हटाएं');
    if (ok != true) return;
    try {
      final token = await AuthService.getToken();
      final res = await ApiService.post('/admin/staff/bulk-unassign',
          {'staffIds': boothIds}, token: token);
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        if (!cScroll.hasListeners) {
          cScroll.addListener(() {
            if (cScroll.position.pixels >= cScroll.position.maxScrollExtent - 150)
              loadCenters(ss: ss);
          });
        }
  ...
  Future<void> _processCSV(List<int> bytes) async {
    ...
    try {
      ...
      _snack('CSV encoding त्रुटि: ${_msg(e)}', error: true);
      return;
    }
    ...
    if (mobileFixed > 0 || mobileInvalid > 0 || rankConverted > 0) {
      final parts = <String>[];
      if (mobileFixed > 0)   parts.add('📱 $mobileFixed ठीक');
      if (mobileInvalid > 0) parts.add('📱 $mobileInvalid अमान्य');
      if (rankConverted > 0) parts.add('🎖 $rankConverted रैंक');
      _snack(parts.join(' • '), error: mobileInvalid > 0);
    }
  ...
  Future<void> _processExcel(List<int> bytes) async {
    ...
    String cs(int ri, int ci) {
      ...
      return normalizeCell(row[ci]?.value);
    }
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        ...
        bool isRowValid(Map<String, dynamic> r) {
          ...
          if ((r['_mobileOk'] as bool?) == false) return false;
          return true;
        }

        final valid = workRows.where(isRowValid).length;
        final mobileBad = workRows.where((r) => (r['_mobileOk'] as bool?) == false).length;
        final armedCount = workRows.where((r) => r['is_armed'] == 1).length;
                ...
                Flexible(
                  child: pageRows.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (_, i) {
                            final r = pageRows[i];
                            final isOk = isRowValid(r);
                            final armed = r['is_armed'] == 1;
                            final mobileOk = (r['_mobileOk'] as bool?) ?? true;
                            return Container(
                ...
                child: Column(
                  ...
                  Wrap(spacing: 8, runSpacing: 3, children: [
                    ...
                    if (_v(s['district']).isNotEmpty)
                      _tag(Icons.location_city_outlined, _v(s['district'])),
      ...
      Column(children: [
          ...
          child: Row(children: [
            ...
            AnimatedBuilder(
              ...
              builder: (_, __) {
                ...
                if (up.isActive) {
                  ...
                  return Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      SizedBox(width: 14, height: 14,
                          child: CircularProgressIndicator(
                              value: overall, color: _kBorder, strokeWidth: 2)),
                      const SizedBox(width: 6),
                ...
                return _actionBtn(Icons.upload_file_outlined,
                    isWide ? 'Upload' : '', _kDark, _pickFile);
              },
            ),
          ]),
        ),
        ...
        Container(
          ...
          child: SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(children: [
              const Icon(Icons.shield_outlined, size: 13, color: _kSubtle),
              const SizedBox(width: 6),
              ...['All', 'Armed', 'Unarmed'].map((opt) {
                final isSel = _armedFilter == opt;
                final color = opt == 'Armed' ? _kArmed
                    : opt == 'Unarmed' ? _kUnarmed : _kSubtle;
