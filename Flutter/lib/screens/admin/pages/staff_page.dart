  ...
  Widget _miniPhase(String label, double pct, Color color) => Expanded(
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        const Spacer(),
        Text('${(pct * 100).round()}%', style: TextStyle(color: color, fontSize: 9, fontWeight: FontWeight.w700)),
      ]),
...
class _SpinIconState extends State<_SpinIcon> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) => RotationTransition(turns: _c, child: Icon(Icons.upload_rounded, color: widget.color, size: 18));
}
...
class _StaffPageState extends State<StaffPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAssigned({bool reset = false}) async {
    ...
    try {
      ...
      final res = await ApiService.get(
        ...
        '&q=${Uri.encodeComponent(_q)}&rank=${Uri.encodeComponent(_rankParam)}',
        token: token);
  ...
  String _msg(Object e) { final s = e.toString(); return s.contains('Exception:') ? s.split('Exception:').last.trim() : s; }
  ...
  Future<void> _removeDuty(Map s) async {
    ...
    final ok = await _confirm('ड्यूटी हटाएं',
        '"${_v(s['name'])}" को ${_v(s['assignLabel'])} से हटाकर रिज़र्व में करें?', 'रिज़र्व करें');
    if (ok != true) return;
    try {
      final token = await AuthService.getToken();
      if (s['dutyId'] != null) {
  ...
  // ── Multi-select ──────────────────────────────────────────────────────────
  void _toggleSelect(int id) => setState(() { if (_selected.contains(id)) _selected.remove(id); else _selected.add(id); });
  void _selectAll() { setState(() { final l = _tabs.index == 0 ? _assigned : _reserve; for (final s in l) _selected.add(s['id'] as int); }); }
  void _clearSelection() => setState(() => _selected.clear());

  Future<void> _bulkDelete() async {
    final count = _selected.length;
    final ok = await _confirm('$count स्टाफ हटाएं', 'चुने गए $count स्टाफ को स्थायी रूप से हटाएं?', 'हटाएं');
    if (ok != true) return;
    try {
      final token = await AuthService.getToken();
      final res = await ApiService.post('/admin/staff/bulk-delete', {'staffIds': _selected.toList()}, token: token);
  ...
  Future<void> _bulkUnassign() async {
    ...
    final boothIds = currentList
        ...
        .toList();
    if (boothIds.isEmpty) { _snack('केवल बूथ स्टाफ ही हटाए जा सकते हैं', error: true); return; }
    final ok = await _confirm('${boothIds.length} ड्यूटी हटाएं', '${boothIds.length} बूथ स्टाफ रिज़र्व में जाएंगे।', 'हटाएं');
    if (ok != true) return;
    try {
      final token = await AuthService.getToken();
      final res = await ApiService.post('/admin/staff/bulk-unassign', {'staffIds': boothIds}, token: token);
      _snack('${res['data']?['removed'] ?? 0} बूथ स्टाफ रिज़र्व में भेजे गए');
      _clearSelection(); _refresh();
    } catch (e) { _snack(_msg(e), error: true); }
  }
  ...
  void _bulkAssignDialog() {
    ...
    showDialog(context: context, barrierDismissible: false,
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        ...
        return Dialog(backgroundColor: Colors.transparent, insetPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 20),
          child: ConstrainedBox(constraints: BoxConstraints(maxWidth: 500, maxHeight: MediaQuery.of(ctx).size.height * 0.85),
            child: Container(decoration: _dlgDec(), child: Column(children: [
              ...
              Flexible(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: BoxDecoration(color: _kPrimary.withOpacity(0.08), borderRadius: BorderRadius.circular(10), border: Border.all(color: _kBorder.withOpacity(0.4))),
                  child: Row(children: [const Icon(Icons.people_outline, size: 16, color: _kPrimary), const SizedBox(width: 8), Text('${selectedIds.length} स्टाफ चुने गए', style: const TextStyle(color: _kPrimary, fontWeight: FontWeight.w700, fontSize: 13))])),
                ...
                const SizedBox(height: 8),
                Container(height: 200, decoration: BoxDecoration(border: Border.all(color: _kBorder), borderRadius: BorderRadius.circular(10), color: Colors.white),
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return InkWell(onTap: () => ss(() => selectedCenter = Map<String, dynamic>.from(c)),
                                child: AnimatedContainer(duration: const Duration(milliseconds: 120), margin: const EdgeInsets.fromLTRB(6, 3, 6, 3), padding: const EdgeInsets.all(10),
                                  decoration: BoxDecoration(color: isSel ? _kPrimary.withOpacity(0.08) : Colors.transparent, borderRadius: BorderRadius.circular(8), border: Border.all(color: isSel ? _kPrimary : _kBorder.withOpacity(0.4), width: isSel ? 1.5 : 1)),
                                  child: Row(children: [
                                    ...
                                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      ...
                                      Text('${_v(c['thana'])} • ${_v(c['gpName'])}', style: const TextStyle(color: _kSubtle, fontSize: 10), maxLines: 1, overflow: TextOverflow.ellipsis),
                                    ])),
                ...
                TextField(controller: busCtrl, style: const TextStyle(color: _kDark, fontSize: 13),
                  decoration: InputDecoration(hintText: 'बस नंबर', hintStyle: const TextStyle(color: _kSubtle, fontSize: 12), prefixIcon: const Icon(Icons.directions_bus_outlined, size: 18, color: _kPrimary),
              ...
              Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 16), child: Row(children: [
                ...
                Expanded(child: ElevatedButton(
                  ...
                  onPressed: selectedCenter == null || saving ? null : () async {
                    ...
                    try {
                      final token = await AuthService.getToken();
                      final res = await ApiService.post('/admin/staff/bulk-assign', {'staffIds': selectedIds, 'centerId': selectedCenter!['id'], 'busNo': busCtrl.text.trim()}, token: token);
                      if (ctx.mounted) Navigator.pop(ctx);
                      _snack('${res['data']?['assigned'] ?? 0} स्टाफ असाइन किए गए');
                      _clearSelection(); _refresh();
                    } catch (e) { ss(() => saving = false); _snack(_msg(e), error: true); }
                  },
                  child: saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : Text('${selectedIds.length} असाइन करें', style: const TextStyle(fontWeight: FontWeight.w700)))),
              ])),
            ]))));
      }));
  }
  ...
  Future<bool?> _confirm(String title, String content, String confirmText) =>
      showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [const Icon(Icons.warning_amber_rounded, color: _kError, size: 20), const SizedBox(width: 8), Expanded(child: Text(title, style: const TextStyle(color: _kError, fontWeight: FontWeight.w800, fontSize: 15)))]),
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: _kError, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), child: Text(confirmText)),
        ]));
  ...
  void _showAssignDialog(Map staff) {
    ...
    showDialog(context: context, barrierDismissible: false,
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        ...
        if (centerList.isEmpty && !cLoading) loadCenters(reset: true, ss: ss);
        return Dialog(backgroundColor: Colors.transparent, insetPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 20),
          child: ConstrainedBox(constraints: BoxConstraints(maxWidth: 500, maxHeight: MediaQuery.of(ctx).size.height * 0.88),
            child: Container(decoration: _dlgDec(), child: Column(children: [
              ...
              Flexible(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (selectedCenter != null) ...[_selectedCenterCard(selectedCenter!, onClear: () => ss(() => selectedCenter = null)), const SizedBox(height: 10)],
                _sectionLabel('मतदान केंद्र चुनें'), const SizedBox(height: 8),
                TextField(onChanged: (v) { cTimer?.cancel(); cTimer = Timer(const Duration(milliseconds: 350), () { centerQ = v; loadCenters(reset: true, ss: ss); }); }, style: const TextStyle(color: _kDark, fontSize: 13), decoration: _searchDec('केंद्र, थाना, GP से खोजें...')),
                const SizedBox(height: 8),
                Container(height: 220, decoration: BoxDecoration(border: Border.all(color: _kBorder), borderRadius: BorderRadius.circular(10), color: Colors.white),
                  child: cLoading && centerList.isEmpty
                    ? const Center(child: CircularProgressIndicator(color: _kPrimary, strokeWidth: 2))
                    : centerList.isEmpty
                        ? const Center(child: Text('कोई केंद्र नहीं मिला', style: TextStyle(color: _kSubtle, fontSize: 12)))
              ...
              Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 16), child: Row(children: [
                ...
                Expanded(child: ElevatedButton(
                  ...
                  onPressed: selectedCenter == null || saving ? null : () async {
                    ss(() => saving = true);
                    try {
                      await ApiService.post('/admin/duties', {'staffId': staff['id'], 'centerId': selectedCenter!['id'], 'busNo': busCtrl.text.trim()}, token: await AuthService.getToken());
                      if (ctx.mounted) Navigator.pop(ctx);
  ...
  void _showEditDialog(Map s) {
    ...
    showDialog(context: context, barrierDismissible: false,
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) => Dialog(backgroundColor: Colors.transparent,
        child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 460),
          child: Container(decoration: _dlgDec(), child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            _dlgActions(ctx, saving, onSave: () async {
              ...
              ss(() => saving = true);
              try {
                await ApiService.put('/admin/staff/${s['id']}', {'name': nc.text.trim(), 'pno': pc.text.trim(), 'mobile': mc.text.trim(), 'thana': tc.text.trim(), 'rank': rc.text.trim()}, token: await AuthService.getToken());
                if (ctx.mounted) Navigator.pop(ctx);
        ...
        child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 460),
          child: Container(decoration: _dlgDec(), child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            _dlgActions(ctx, saving, saveLabel: 'जोड़ें', onSave: () async {
              ...
              ss(() => saving = true);
              try {
                await ApiService.post('/admin/staff', {'pno': pc.text.trim(), 'name': nc.text.trim(), 'mobile': mc.text.trim(), 'thana': tc.text.trim(), 'district': dc.text.trim(), 'rank': rc.text.trim()}, token: await AuthService.getToken());
                if (ctx.mounted) Navigator.pop(ctx);
    ...
    try {
      ...
      await for (final raw in resp.stream.transform(utf8.decoder)) {
        ...
        while (buf.contains('\n')) {
          ...
          if (phase == 'parse')       { up.update(p: _UploadPhase.uploading, pp: (pct / 100.0).clamp(0, 1), msg: data['msg'] as String? ?? '...'); }
          else if (phase == 'hash')   { up.update(pp: 1.0, hp: ((pct - 25.0) / 30.0).clamp(0, 1), msg: data['msg'] as String? ?? '...'); }
          else if (phase == 'insert') { up.update(pp: 1.0, hp: 1.0, ip: ((pct - 55.0) / 43.0).clamp(0, 1), a: (data['added'] as num?)?.toInt() ?? 0, t: (data['total'] as num?)?.toInt() ?? toUpload.length, msg: '${data['added'] ?? 0}/${data['total'] ?? toUpload.length} rows'); }
          else if (phase == 'done')   { final added = (data['added'] as num?)?.toInt() ?? 0; final skipped = (data['skipped'] as List?)?.length ?? 0; up.update(p: _UploadPhase.done, pp: 1.0, hp: 1.0, ip: 1.0, a: added, msg: '$added जोड़े गए, $skipped छोड़े गए'); client.close(); return; }
          else if (phase == 'error')  { throw Exception(data['message'] as String? ?? 'Server error'); }
        }
      }
    } catch (e) { client?.close(); UploadProgress.instance.update(p: _UploadPhase.error, msg: _msg(e), err: _msg(e)); }
  }

  Future<void> _pickExcel() async {
    if (mounted) setState(() => _excelLoading = true);
    FilePickerResult? result;
    try { result = await FilePicker.platform.pickFiles(type: FileType.custom, allowedExtensions: ['xlsx', 'xls'], withData: true); }
    catch (e) { if (mounted) setState(() => _excelLoading = false); _snack('File picker: ${_msg(e)}', error: true); return; }
    ...
    String cs(int ri, int ci) { if (ri >= sheet.rows.length) return ''; final row = sheet.rows[ri]; if (ci >= row.length) return ''; return (row[ci]?.value?.toString() ?? '').trim(); }
    ...
    final preview = <Map<String, dynamic>>[]; const chunk = 500;
    ...
    for (int ri = dataStart; ri < sheet.rows.length; ri += chunk) {
      ...
      for (int r = ri; r < end; r++) {
        ...
        preview.add({'pno': pno, 'name': name, 'mobile': cs(r, iMob!), 'thana': cs(r, iThana!), 'district': cs(r, iDist!), 'rank': cs(r, iRank!), '_row': r + 1});
      }
      await Future.delayed(Duration.zero);
      UploadProgress.instance.update(pp: (0.2 + ((end - dataStart) / totalRows.clamp(1, 999999)) * 0.8).clamp(0, 1), a: preview.length, msg: '${preview.length} rows मिले...');
    }
  ...
  Future<String?> _pickSheet(List<String> names) => showDialog<String>(context: context,
    builder: (ctx) => AlertDialog(backgroundColor: _kBg,
  ...
  void _showExcelPreview(List<Map<String, dynamic>> initial) {
    ...
    showDialog(context: context, barrierDismissible: false,
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        final filtered = previewQ.isEmpty ? workRows : workRows.where((r) { final q = previewQ.toLowerCase(); return (r['name'] as String? ?? '').toLowerCase().contains(q) || (r['pno'] as String? ?? '').toLowerCase().contains(q) || (r['thana'] as String? ?? '').toLowerCase().contains(q); }).toList();
        final totalPages = ((filtered.length - 1) ~/ ppSize) + 1;
        final sp = previewPage.clamp(1, totalPages.clamp(1, 9999));
        final ps = (sp - 1) * ppSize; final pe = (ps + ppSize).clamp(0, filtered.length);
        ...
        return Dialog(backgroundColor: Colors.transparent, insetPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 16),
          child: ConstrainedBox(constraints: BoxConstraints(maxWidth: 560, maxHeight: MediaQuery.of(ctx).size.height * 0.92),
            child: Container(decoration: _dlgDec(), child: Column(mainAxisSize: MainAxisSize.min, children: [
              _dlgHeader('Preview — ${workRows.length}/${allRows.length} rows', Icons.upload_file_outlined, ctx),
              Padding(padding: const EdgeInsets.fromLTRB(14, 10, 14, 4), child: Row(children: [_pill('$valid मान्य', _kSuccess), const SizedBox(width: 8), _pill('${workRows.length - valid} त्रुटि', _kError), const Spacer(), const Icon(Icons.touch_app_outlined, size: 11, color: _kSubtle), const SizedBox(width: 3), const Text('× से हटाएं', style: TextStyle(color: _kSubtle, fontSize: 10))])),
              Padding(padding: const EdgeInsets.fromLTRB(12, 4, 12, 6), child: TextField(controller: psCtrl, style: const TextStyle(color: _kDark, fontSize: 13), onChanged: (v) { pdebounce?.cancel(); pdebounce = Timer(const Duration(milliseconds: 250), () { ss(() { previewQ = v.trim(); previewPage = 1; }); }); }, decoration: _searchDec('नाम, PNO, थाना से खोजें...', onClear: previewQ.isNotEmpty ? () { psCtrl.clear(); ss(() { previewQ = ''; previewPage = 1; }); } : null))),
                ...
                : ListView.builder(shrinkWrap: true, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                    itemCount: pageRows.length, itemBuilder: (_, i) {
                      ...
                      return Container(margin: const EdgeInsets.only(bottom: 6),
                        ...
                        child: Row(children: [
                          Container(width: 36, alignment: Alignment.center, padding: const EdgeInsets.symmetric(vertical: 10), decoration: BoxDecoration(color: isOk ? _kSurface.withOpacity(0.6) : _kError.withOpacity(0.06), borderRadius: const BorderRadius.only(topLeft: Radius.circular(9), bottomLeft: Radius.circular(9))), child: Text('${r['_row']}', style: TextStyle(color: isOk ? _kSubtle : _kError, fontSize: 10, fontWeight: FontWeight.w700))),
                          ...
                          InkWell(onTap: () => ss(() { workRows.remove(r); final nf = previewQ.isEmpty ? workRows : workRows.where((x) => (x['name'] as String? ?? '').toLowerCase().contains(previewQ.toLowerCase()) || (x['pno'] as String? ?? '').toLowerCase().contains(previewQ.toLowerCase())).toList(); final ntp = ((nf.length - 1) ~/ ppSize).clamp(0, 9999) + 1; if (previewPage > ntp) previewPage = ntp.clamp(1, 9999); }), borderRadius: const BorderRadius.only(topRight: Radius.circular(9), bottomRight: Radius.circular(9)), child: Container(width: 36, height: 52, alignment: Alignment.center, child: const Icon(Icons.close, size: 15, color: _kError))),
                        ]));
                    })),
              if (totalPages > 1)
                Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: _kSurface.withOpacity(0.5), border: Border(top: BorderSide(color: _kBorder.withOpacity(0.3)))),
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    _pageBtn(Icons.chevron_left, sp > 1, () => ss(() => previewPage = sp - 1)), const SizedBox(width: 8),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5), decoration: BoxDecoration(color: _kPrimary.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: _kBorder.withOpacity(0.4))), child: Text('$sp / $totalPages  (${filtered.length} rows)', style: const TextStyle(color: _kDark, fontSize: 12, fontWeight: FontWeight.w700))),
                    const SizedBox(width: 8), _pageBtn(Icons.chevron_right, sp < totalPages, () => ss(() => previewPage = sp + 1)),
                  ])),
              Padding(padding: const EdgeInsets.fromLTRB(14, 8, 14, 16), child: Row(children: [
                Expanded(child: OutlinedButton(onPressed: () => Navigator.pop(ctx), style: OutlinedButton.styleFrom(foregroundColor: _kSubtle, side: const BorderSide(color: _kBorder), padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), child: const Text('रद्द'))),
                const SizedBox(width: 12),
                Expanded(child: ElevatedButton.icon(
                  ...
                  onPressed: valid == 0 ? null : () { Navigator.pop(ctx); final toUpload = workRows.where((r) => (r['pno'] as String? ?? '').isNotEmpty && (r['name'] as String? ?? '').isNotEmpty).map((r) { final m = Map<String, dynamic>.from(r)..remove('_row'); return m; }).toList(); _startBackgroundUpload(toUpload); },
                  icon: const Icon(Icons.upload, size: 16), label: Text('$valid अपलोड करें'))),
              ])),
            ]))));
      }));
  }
  ...
  Widget _assignmentChip(Map s) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: config.$2.withOpacity(0.06), borderRadius: BorderRadius.circular(6), border: Border.all(color: config.$2.withOpacity(0.2))),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(config.$1, size: 11, color: config.$2), const SizedBox(width: 4),
        Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1), decoration: BoxDecoration(color: config.$2.withOpacity(0.12), borderRadius: BorderRadius.circular(4)), child: Text(config.$3, style: TextStyle(color: config.$2, fontSize: 9, fontWeight: FontWeight.w800))),
        const SizedBox(width: 5),
        Flexible(child: Text(label, style: TextStyle(color: config.$2, fontSize: 11, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis)),
        if (detail.isNotEmpty) ...[
          Text('  •  ', style: TextStyle(color: config.$2.withOpacity(0.5), fontSize: 10)),
          Flexible(child: Text(detail, style: TextStyle(color: config.$2.withOpacity(0.7), fontSize: 10), maxLines: 1, overflow: TextOverflow.ellipsis)),
        ],
      ]),
    );
  }
  ...
  Widget _staffCard(Map s, {required bool assigned}) {
    ...
    final isOfficer   = assignType != '' && assignType != 'booth';
        ...
        child: AnimatedContainer(
          ...
          child: Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              // Avatar / Checkbox
              GestureDetector(onTap: () => _toggleSelect(id),
                child: AnimatedSwitcher(duration: const Duration(milliseconds: 200),
                  child: _selectMode
                    ? Container(key: const ValueKey('cb'), width: 44, height: 44,
                        decoration: BoxDecoration(shape: BoxShape.circle, color: isSelected ? _kPrimary : Colors.white, border: Border.all(color: isSelected ? _kPrimary : _kBorder, width: 2)),
                        child: Icon(isSelected ? Icons.check : null, color: Colors.white, size: 22))
                    : Container(key: const ValueKey('av'), width: 44, height: 44,
                        decoration: BoxDecoration(shape: BoxShape.circle, color: avatarColor.withOpacity(0.12), border: Border.all(color: avatarColor.withOpacity(0.35))),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (_v(s['assignLabel']).isNotEmpty) ...[const SizedBox(height: 5), _assignmentChip(s)],
              ])),
              ...
              Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(height: 4),
                // 3rd button: booth → remove duty, reserve → assign, officer → locked
                if (!assigned)
                  _iconBtn(Icons.how_to_vote_outlined, _kPrimary, () => _showAssignDialog(s))
  ...
  Widget _selectionBar() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: _kDark, borderRadius: BorderRadius.circular(12),
          boxShadow: [BoxShadow(color: _kDark.withOpacity(0.3), blurRadius: 12, offset: const Offset(0, 4))]),
      child: Row(children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: _kBorder.withOpacity(0.25), borderRadius: BorderRadius.circular(20)),
        ...
        const Spacer(),
        if (!isAssignedTab) ...[_miniActionBtn('असाइन', Icons.how_to_vote_outlined, _kBorder, _bulkAssignDialog), const SizedBox(width: 6)],
        if (isAssignedTab)  ...[_miniActionBtn('रिज़र्व', Icons.person_remove_outlined, _kAccent, _bulkUnassign), const SizedBox(width: 6)],
        _miniActionBtn('हटाएं', Icons.delete_outline, _kError, _bulkDelete),
        const SizedBox(width: 6),
        GestureDetector(onTap: _clearSelection,
            child: Container(padding: const EdgeInsets.all(7), decoration: BoxDecoration(color: Colors.white12, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.close, size: 16, color: Colors.white70))),
      ]),
    );
  }
  ...
  Widget _miniActionBtn(String label, IconData icon, Color color, VoidCallback onTap) =>
      GestureDetector(onTap: onTap, child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 13, color: color), const SizedBox(width: 4), Text(label, style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w700))])));
      ...
      Column(children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 6),
            AnimatedBuilder(animation: UploadProgress.instance,
              builder: (_, __) {
                final up = UploadProgress.instance;
                if (_excelLoading || up.phase == _UploadPhase.parsing) {
                  return Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 9), decoration: BoxDecoration(color: _kDark, borderRadius: BorderRadius.circular(10)),
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [SizedBox(width: 14, height: 14, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)), SizedBox(width: 6), Text('लोड...', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700))]));
                }
                if (up.isActive) {
                  final overall = ((up.parsePct * 0.15) + (up.hashPct * 0.30) + (up.insertPct * 0.55)).clamp(0.0, 1.0);
                  return Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 9), decoration: BoxDecoration(color: _kDark, borderRadius: BorderRadius.circular(10)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [SizedBox(width: 14, height: 14, child: CircularProgressIndicator(value: overall, color: _kBorder, strokeWidth: 2)), const SizedBox(width: 6), Text('${(overall * 100).round()}%', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700))]));
                }
        ...
        // ── RANK FILTER CHIPS (scrollable row) ────────────────────────────
        Container(
          color: _kBg,
          padding: const EdgeInsets.fromLTRB(12, 6, 12, 6),
          child: SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(children: _kAllRanks.map((rank) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: isSel ? [BoxShadow(color: _kPrimary.withOpacity(0.2), blurRadius: 4, offset: const Offset(0, 2))] : [],
                  ),
                  child: Text(rank,
                    style: TextStyle(
        ...
        // ── SUMMARY CHIPS ─────────────────────────────────────────────────
        Container(color: _kBg, padding: const EdgeInsets.fromLTRB(12, 4, 12, 6),
          child: Row(children: [
            ...
            const Spacer(),
            if (_q.isNotEmpty || _selectedRank != 'All')
              Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: _kInfo.withOpacity(0.08), borderRadius: BorderRadius.circular(6), border: Border.all(color: _kInfo.withOpacity(0.2))),
                child: Text('फ़िल्टर सक्रिय', style: const TextStyle(color: _kInfo, fontSize: 10, fontWeight: FontWeight.w700))),
        ...
        Expanded(child: TabBarView(controller: _tabs, children: [
          _buildList(items: _assigned, loading: _assignedLoading, hasMore: _assignedHasMore, scroll: _assignedScroll, assigned: true, emptyMsg: _q.isNotEmpty ? '"$_q" के लिए कोई result नहीं' : 'कोई असाइन स्टाफ नहीं', emptyIcon: Icons.how_to_vote_outlined),
          _buildList(items: _reserve,  loading: _reserveLoading,  hasMore: _reserveHasMore,  scroll: _reserveScroll,  assigned: false, emptyMsg: _q.isNotEmpty ? '"$_q" के लिए कोई result नहीं' : 'सभी स्टाफ असाइन हैं!', emptyIcon: Icons.badge_outlined),
        ])),
      ]),
  ...
  Widget _buildList({required List<Map> items, required bool loading, required bool hasMore, required ScrollController scroll, required bool assigned, required String emptyMsg, required IconData emptyIcon}) {
    ...
    return RefreshIndicator(onRefresh: () async => _refresh(), color: _kPrimary,
      child: Scrollbar(controller: scroll, thumbVisibility: true, thickness: 6, radius: const Radius.circular(3),
        child: ListView.builder(controller: scroll, padding: const EdgeInsets.fromLTRB(12, 10, 12, 100), addRepaintBoundaries: false,
  ...
  Widget _dlgActions(BuildContext ctx, bool saving, {String saveLabel = 'अपडेट', required VoidCallback onSave}) =>
    Padding(padding: const EdgeInsets.all(20), child: Row(children: [
      Expanded(child: OutlinedButton(onPressed: saving ? null : () => Navigator.pop(ctx), style: OutlinedButton.styleFrom(foregroundColor: _kSubtle, side: const BorderSide(color: _kBorder), padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), child: const Text('रद्द'))),
      const SizedBox(width: 12),
      Expanded(child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: _kPrimary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), onPressed: saving ? null : onSave,
  ...
  Widget _field(TextEditingController c, String label, IconData icon, {bool req = false, TextInputType? type}) =>
    Padding(padding: const EdgeInsets.only(bottom: 10), child: TextFormField(controller: c, keyboardType: type, style: const TextStyle(color: _kDark, fontSize: 13),
      decoration: InputDecoration(labelText: label, labelStyle: const TextStyle(color: _kSubtle, fontSize: 12), prefixIcon: Icon(icon, size: 18, color: _kPrimary), filled: true, fillColor: Colors.white, isDense: true,
  ...
  Widget _summaryChip(String label, String count, Color color) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: color.withOpacity(0.25))),
    child: RichText(text: TextSpan(children: [TextSpan(text: '$count ', style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.w900)), TextSpan(text: label, style: const TextStyle(color: _kSubtle, fontSize: 11))])));
  Widget _actionBtn(IconData icon, String label, Color color, VoidCallback onTap) =>
    GestureDetector(onTap: onTap, child: Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 9), decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(10)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: Colors.white, size: 14), const SizedBox(width: 4), Text(label, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700))])));
  Widget _iconBtn(IconData icon, Color color, VoidCallback onTap) =>
    GestureDetector(onTap: onTap, child: Container(width: 34, height: 34, decoration: BoxDecoration(color: color.withOpacity(0.08), borderRadius: BorderRadius.circular(8), border: Border.all(color: color.withOpacity(0.25))), child: Icon(icon, size: 16, color: color)));
