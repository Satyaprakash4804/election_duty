...
// ══════════════════════════════════════════════════════════════════════════════
//  PALETTE  (matches dashboard_page.dart exactly)
// ══════════════════════════════════════════════════════════════════════════════
...
// ── Helper: rank → Hindi abbreviation ────────────────────────────────────────
...
// ── Helper: armed detection (covers ALL field name variants from backend) ─────
/// Returns true for ANY truthy representation the backend might send:
///   bool true, int 1, String "1"/"yes"/"true"/"सशस्त्र"/"armed"
bool _isArmedValue(dynamic v) {
  if (v == null) return false;
  if (v is bool) return v;
  if (v is int)  return v == 1;
  final s = v.toString().toLowerCase().trim();
  return s == '1' || s == 'yes' || s == 'true' || s == 'armed' || s == 'सशस्त्र';
}

/// Reads armed status from a staff/duty map checking EVERY known key.
bool _staffIsArmed(Map m) =>
    _isArmedValue(m['isArmed'])   ||
    _isArmedValue(m['is_armed'])  ||
    _isArmedValue(m['armed'])     ||
    _isArmedValue(m['sastra'])    ||
    _isArmedValue(m['weapon']);
...
class _ElectionConfig {
  ...
  final String electionDate;   // always DD.MM.YYYY for PDF
  ...
  bool get isEmpty =>
      electionName.isEmpty && district.isEmpty;

  factory _ElectionConfig.fromMap(Map<String, dynamic> m) {
    // Date: camelCase first, then snake_case
    String rawDate =
    ...
    final fmtDate = rawDate.isNotEmpty ? rawDate : '26.04.2024';

    // Year
    String year =
        (m['electionYear'] ?? m['election_year'] ?? '').toString().trim();
    if (year.isEmpty && fmtDate.length >= 4) {
      year = fmtDate.substring(fmtDate.length - 4);
    }
    if (year.isEmpty) year = '2024';

    // Times
    String pratah =
        (m['pratahSamay'] ?? m['pratah_samay'] ?? '07:00').toString().trim();
    String saya =
        (m['sayaSamay'] ?? m['saya_samay'] ?? '06:00').toString().trim();
    if (pratah.isEmpty) pratah = '07:00';
    if (saya.isEmpty)   saya   = '06:00';
    ...
    return _ElectionConfig(
      ...
      phase:        (m['phase']                              ?? 'द्वितीय').toString().trim(),
      electionYear: year,
      electionDate: fmtDate,
      pratahSamay:  pratah,
      sayaSamay:    saya,
    );
  }
...
// ══════════════════════════════════════════════════════════════════════════════
//  BOOTH DUTY CARD PDF  ← FORMAT UNCHANGED
// ══════════════════════════════════════════════════════════════════════════════
pw.Widget buildDutyCardPdf(Map s, pw.Font font, pw.Font bold,
  ...
  final String districtLabel =
      (s['adminDistrict'] ?? '').toString().trim().isNotEmpty
          ...
          : (config['district']?.isNotEmpty == true
              ? config['district']!
              : 'बागपत');
  ...
  final String electionYear = config['electionYear']?.isNotEmpty == true
      ? config['electionYear']!
      : (electionDate.length >= 4
          ? electionDate.substring(electionDate.length - 4)
          : '2024');
  ...
  pw.Widget td(String t, {bool center = false, bool isBold = false, double fs = 5.5}) =>
      pw.Padding(
        padding: const pw.EdgeInsets.symmetric(horizontal: 1, vertical: 1),
        child: pw.Text(t,
  ...
  // ── Main staff armed label (uses _staffIsArmed logic) ──────────────────
  final mainArmed = _staffIsArmed(s as Map);
  final mainArmedLabel = mainArmed ? 'सशस्त्र' : 'निःशस्त्र';
      ...
      pw.Container(
        ...
        child: pw.Row(children: [
          pw.Container(width: 42, padding: const pw.EdgeInsets.all(3),
            ...
            child: pw.Center(child: pw.Text('ECI',
      ...
      pw.Table(
        ...
        children: [
          ...
          pw.TableRow(children: [
            ...
            // ✅ FIXED: use _staffIsArmed helper
            td(mainArmedLabel, center: true, fs: 4.5),
        ...
        child: pw.Row(
            ...
            child: pw.Column(children: [
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  // ✅ FIXED: use _staffIsArmed for each sahyogi row
                  final eArmed = e != null ? _staffIsArmed(e as Map) : false;
                  return pw.Expanded(
                    child: pw.Container(
                      decoration: pw.BoxDecoration(
                        color: i.isEven
          ...
          pw.Container(width: 28,
            ...
            child: pw.Column(children: [
              ...
              pw.Center(child: pw.Text('दिनांक',
                  style: pw.TextStyle(font: bold, fontSize: 5))),
...
// ══════════════════════════════════════════════════════════════════════════════
//  DISTRICT DUTY CARD PDF  ← FORMAT UNCHANGED
// ══════════════════════════════════════════════════════════════════════════════
pw.Widget buildDistrictDutyCardPdf(Map s, pw.Font font, pw.Font bold,
    {Map<String, String> config = const {}}) {
  ...
  return pw.Container(
    ...
    child: pw.Column(
      ...
      pw.Table(
        ...
        children: [
          pw.TableRow(children: [
            th('ड्यूटी प्रकार'), th('बैच सं0'),
            th('बस सं0'), th('दिनांक'), th('कुल कर्मी'),
          ]),
          pw.TableRow(children: [
            pw.Padding(
        ...
        child: pw.Column(children: [
          ...
          pw.Expanded(
            child: pw.Column(
              ...
              // ✅ FIXED: use _staffIsArmed for district duty rows
              final eArmed = e != null ? _staffIsArmed(e as Map) : false;
              return pw.Expanded(
                child: pw.Container(
                  decoration: pw.BoxDecoration(
...
class _DutyCardPageState extends State<DutyCardPage>
  ...
  Map<String, List<dynamic>> _districtBatches = {};
  bool    _districtLoading   = false;
  String? _expandedDutyType;

  // ── Filters ────────────────────────────────────────────────────────────────
  String          _q              = '';
  // ✅ FIX: rank filter stores the EXACT string the API uses (case-sensitive)
  String?         _rankFilter;
  ...
  void initState() {
    ...
    _scroll.addListener(() {
      if (_scroll.position.pixels >=
          _scroll.position.maxScrollExtent - 300) _loadMore();
    });
  ...
  // ── Step 1: admin profile → Step 2: election config ───────────────────────
  Future<void> _initAdminThenConfig() async {
    try {
  ...
  // ── Election config fetch — 3-source cascade (DO NOT CHANGE ORDER) ────────
  //  Source 1: /admin/election-config/active   → { hasActiveConfig, config:{} }
  //  Source 2: embedded in /admin/duties response
  //  Source 3: /admin/config (legacy key-value table)
  Future<void> _fetchConfig() async {
    if (!mounted) return;
    setState(() => _configLoading = true);

    // ── Source 1: /admin/election-config/active ────────────────────────────
    try {
      final token = await AuthService.getToken();
      final res   = await ApiService.get(
          '/admin/election-config/active', token: token);

      // Response shape: { success, data: { hasActiveConfig, config: {...} } }
      ...
      if (hasActive && cfg.isNotEmpty && mounted) {
        final parsed = _ElectionConfig.fromMap(cfg);
        // Only use if district matches (or config has no district)
        if (parsed.district.isEmpty ||
            parsed.district.toLowerCase() ==
                _adminDistrict.toLowerCase()) {
          setState(() {
            _electionConfig = parsed;
      ...
      debugPrint('[DutyCardPage] election-config/active failed: $e');
    }

    // ── Source 2: embedded in /admin/duties page-1 ─────────────────────────
    try {
      final token = await AuthService.getToken();
      final res   = await ApiService.get(
          '/admin/duties?page=1&limit=1', token: token);
      final wrap  = (res['data'] as Map<String, dynamic>?) ?? {};
      final emb   = wrap['electionConfig'];

      if (emb is Map<String, dynamic> && emb.isNotEmpty && mounted) {
        final parsed = _ElectionConfig.fromMap(emb);
        if (parsed.district.isEmpty ||
            parsed.district.toLowerCase() ==
                _adminDistrict.toLowerCase()) {
          setState(() {
      ...
      debugPrint('[DutyCardPage] duties embed failed: $e');
    }

    // ── Source 3: /admin/config (legacy) ────────────────────────────────────
    try {
      final token     = await AuthService.getToken();
      final legacyRes = await ApiService.get('/admin/config', token: token);
      // Shape: { data: { electionConfig: {...}, district, ... } }
      final outer = (legacyRes['data'] as Map<String, dynamic>?) ?? {};
      final ecMap = outer['electionConfig'] as Map<String, dynamic>?;

      if (ecMap != null && ecMap.isNotEmpty && mounted) {
        setState(() {
          _electionConfig = _ElectionConfig.fromMap({
            ...ecMap,
            'district': _adminDistrict,
          });
          _configLoading = false;
        });
        return;
      ...
      if (outer.isNotEmpty && mounted) {
        setState(() {
          _electionConfig = _ElectionConfig(
            district:     _adminDistrict,
            phase:
                (outer['phase']        ?? 'द्वितीय').toString(),
            electionDate:
                (outer['electionDate'] ?? '26.04.2024').toString(),
            electionYear:
                (outer['electionYear'] ?? '2024').toString(),
            electionName:
                (outer['electionName'] ?? '').toString(),
            state:
                (outer['state']        ?? '').toString(),
            pratahSamay:
                (outer['pratahSamay']  ?? '07:00').toString(),
            sayaSamay:
                (outer['sayaSamay']    ?? '06:00').toString(),
          );
          _configLoading = false;
        });
        return;
      ...
      debugPrint('[DutyCardPage] /admin/config failed: $e');
    }
  ...
  Future<void> _loadDistrictDutySummary() async {
    ...
    try {
      final token = await AuthService.getToken();
      final res   = await ApiService.get(
          '/admin/district-duty/summary', token: token);
      if (!mounted) return;
      setState(() {
        _districtSummary =
  ...
  Future<void> _fetch() async {
    ...
    try {
      ...
      // Opportunistically fill config from embedded response
      if (_electionConfig.isEmpty && !_configLoading) {
        final emb = wrapper['electionConfig'];
        if (emb is Map<String, dynamic> && emb.isNotEmpty) {
          final parsed = _ElectionConfig.fromMap(emb);
          if (parsed.district.isEmpty ||
              parsed.district.toLowerCase() ==
                  _adminDistrict.toLowerCase()) {
            if (mounted) setState(() => _electionConfig = parsed);
          }
        }
      }
  ...
  // ✅ FIX: Rank filter now does case-insensitive comparison on BOTH
  //    the primary staff rank AND every sahyogi (helper) rank.
  List<Map<String, dynamic>> get _visible => _items.where((s) {
    // ── Rank filter ──────────────────────────────────────────────────────────
    if (_rankFilter != null && _rankFilter!.isNotEmpty) {
      final rf = _rankFilter!.toLowerCase();
      // Check primary staff rank
      final primaryRank =
          (s['rank'] ?? s['user_rank'] ?? '').toString().toLowerCase().trim();
      final primaryMatch = primaryRank == rf;
      // Check sahyogi ranks
      final sahyogiMatch =
          ((s['sahyogi'] ?? s['allStaff'] ?? []) as List).any((e) {
        final er =
            (e['user_rank'] ?? e['rank'] ?? '').toString().toLowerCase().trim();
        return er == rf;
      });
      if (!primaryMatch && !sahyogiMatch) return false;
    }

    // ── Armed filter ─────────────────────────────────────────────────────────
    // ✅ FIX: use _staffIsArmed which checks ALL known field variants
    if (_armedFilter != _ArmedFilter.all) {
      final want    = _armedFilter == _ArmedFilter.armed;
      final isArmed = _staffIsArmed(s);
      if (want != isArmed) return false;
    }

    // ── Download filter ───────────────────────────────────────────────────────
    if (_downloadFilter != _DownloadFilter.all) {
      final dl = s['cardDownloaded'] == true;
      if (_downloadFilter == _DownloadFilter.downloaded    && !dl) return false;
      if (_downloadFilter == _DownloadFilter.notDownloaded &&  dl) return false;
    }
  ...
  String   _armedLabel(_ArmedFilter f)    =>
      const ['सभी', 'सशस्त्र', 'निःशस्त्र'][f.index];
  Color    _armedColor(_ArmedFilter f)    =>
      [kPrimary, const Color(0xFFC62828), const Color(0xFF1565C0)][f.index];
  IconData _armedIcon(_ArmedFilter f)     =>
      [Icons.people_outline, Icons.shield_outlined, Icons.person_outline][f.index];
  String   _dlLabel(_DownloadFilter f)    =>
      ['सभी', 'डाउनलोड', 'शेष'][f.index];
  Color    _dlColor(_DownloadFilter f)    =>
      [kPrimary, kSuccess, const Color(0xFFE65100)][f.index];
  IconData _dlIcon(_DownloadFilter f)     =>
      [Icons.list_outlined, Icons.check_circle_outline, Icons.pending_outlined][f.index];
  ...
  Future<void> _printDistrictBatch(
      Map batchData, String dutyLabelHi) async {
    final pdf       = pw.Document();
    final font      = await PdfGoogleFonts.notoSansDevanagariRegular();
    final bold      = await PdfGoogleFonts.notoSansDevanagariBold();
    final cfg       = _electionConfig.toConfigMap();
    ...
    pdf.addPage(pw.Page(
      ...
      margin:     const pw.EdgeInsets.all(4),
      build:      (_) => buildDistrictDutyCardPdf(
          {...batchData, 'dutyLabelHi': dutyLabelHi}, font, bold, config: cfg),
    ));
    await Printing.layoutPdf(onLayout: (_) => pdf.save());
  }

  Future<void> _printAllDistrictBatches(
      String dutyType, String dutyLabelHi) async {
    ...
    for (final batch in batches) {
      ...
      pdf.addPage(pw.Page(
        ...
        margin:     const pw.EdgeInsets.all(4),
        build:      (_) => buildDistrictDutyCardPdf(
            {...Map<String, dynamic>.from(batch as Map),
             'dutyLabelHi': dutyLabelHi},
            font, bold, config: cfg),
      ));
    }
    await Printing.layoutPdf(onLayout: (_) => pdf.save());
  }
  ...
  Future<void> _printAll() async {
    ...
    try {
      ...
      // Apply current filters to the complete list
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Expanded(
        child: TabBarView(
          controller: _tabCtrl,
          children: [_buildBoothTab(sw), _buildDistrictTab(sw)],
        ),
      ),
    ]);
  }
  ...
  Widget _buildConfigBanner(double sw) {
    if (_configLoading) {
      return Container(
        color: kSurface,
        ...
        child: const Row(children: [
          ...
          SizedBox(width: 10),
          Text('निर्वाचन विवरण लोड हो रहा है...',
              style: TextStyle(color: kSubtle, fontSize: 11)),
        ]),
      );
    }

    if (_electionConfig.isEmpty) {
      return Container(
    ...
    // Config loaded — show compact banner
    final districtDisplay = _electionConfig.district.isNotEmpty
        ? _electionConfig.district
        : _adminDistrict;
    ...
    return Container(
      ...
      padding: EdgeInsets.symmetric(
          horizontal: sw > 600 ? 16 : 12, vertical: 7),
      child: Row(children: [
        const Icon(Icons.how_to_vote, color: kPrimary, size: 15),
        const SizedBox(width: 8),
        Expanded(
          child: Column(
              crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(
              _electionConfig.electionName.isNotEmpty
                  ? _electionConfig.electionName
                  : 'निर्वाचन',
              style: const TextStyle(
                  color: kDark, fontSize: 11, fontWeight: FontWeight.w800),
              maxLines: 1, overflow: TextOverflow.ellipsis,
            ),
            const SizedBox(height: 1),
            Text(
              'जनपद: $districtDisplay'
              '  •  चरण: ${_electionConfig.phase}'
              '  •  ${_electionConfig.electionDate}',
        ...
        Container(
          ...
          child: Text(
            '${_electionConfig.pratahSamay}–${_electionConfig.sayaSamay}',
            style: const TextStyle(
                color: kPrimary, fontSize: 9, fontWeight: FontWeight.w700),
          ),
        ),
        const SizedBox(width: 4),
        GestureDetector(
          onTap: _fetchConfig,
          child: const Icon(Icons.refresh, size: 14, color: kSubtle),
        ),
      ]),
    );
  }
  ...
  //  BOOTH TAB
  // ══════════════════════════════════════════════════════════════════════════
  Widget _buildBoothTab(double sw) {
    ...
    return Column(children: [
      ...
      Container(
        ...
        padding: EdgeInsets.fromLTRB(
            isWide ? 16 : 12, 10, isWide ? 16 : 12, 10),
        child: Column(children: [
          ...
          // ✅ FIX: Rank chips — use exact strings matching API 'rank'/'user_rank'
          // SizedBox(
          //   height: 32,
          //   child: ListView(
          ...
          //         onTap: () {
          //           if (_rankFilter != null) {
          //             setState(() => _rankFilter = null);
          //           }
          //         },
          //       ),
          //       const SizedBox(width: 6),
          //       ..._kAllRanks.map((rank) {
          //         final sel = _rankFilter == rank;
          //         return Padding(
      ...
      if (visible.isNotEmpty)
        Container(
          color: kBg,
          padding: EdgeInsets.symmetric(
              horizontal: isWide ? 16 : 12, vertical: 8),
          child: Row(children: [
            Flexible(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (_selected.isNotEmpty) ...[
              _ActionBtn(
                ...
                onTap: () {
                  final sel = visible
                      .where((s) =>
                          _selected.contains(s['id'] as int))
            ...
            TextButton(
              onPressed: () => setState(() {
                if (_selected.length == visible.length) {
                  ...
                  _selected =
                      visible.map((s) => s['id'] as int).toSet();
                }
              }),
      ...
      if (_loading && _items.isEmpty)
        ...
        Expanded(child: emptyState(_buildEmptyLabel(),
        ...
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (ctx, i) {
              ...
              final primaryRank =
                  (s['rank'] ?? s['user_rank'] ?? '').toString();
              final rankHindi = _rh(primaryRank);

              // ✅ FIX: use _staffIsArmed which checks all field variants
              final isArmed    = _staffIsArmed(s);
                ...
                child: AnimatedContainer(
                  ...
                  child: ListTile(
                    ...
                    title: Row(children: [
                      ...
                      // ✅ FIXED: armed/unarmed badge uses _staffIsArmed
                      _ArmedBadge(isArmed: isArmed),
                      const SizedBox(width: 4),
  ...
  //  DISTRICT TAB
  // ══════════════════════════════════════════════════════════════════════════
  Widget _buildDistrictTab(double sw) {
    final isWide = sw > 600;
    ...
    if (_districtSummary.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          emptyState('कोई जनपदीय ड्यूटी नहीं मिली',
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) {
        ...
        final labelHi    =
            (info['dutyLabelHi'] ?? dutyType).toString();
        ...
        final pct        = sankhya == 0
            ? 0.0
            : (assigned / sankhya).clamp(0.0, 1.0);
          ...
          child: Column(
            ...
            InkWell(
              ...
              onTap: () async {
                ...
                if (!isExpanded &&
                    !_districtBatches.containsKey(dutyType)) {
                  ...
                  Row(children: [
                    ...
                    if (batchCount > 0)
                      IconButton(
                        ...
                        onPressed: () async {
                          if (!_districtBatches
                              .containsKey(dutyType)) {
                            await _loadBatchesForDutyType(
                                dutyType);
                          }
                          await _printAllDistrictBatches(
                              dutyType, labelHi);
                        },
                      ),
                    Icon(
                    ...
                    child: Column(
                      ...
                      if (note.isNotEmpty)
                        Padding(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 12),
                            child: _tag(Icons.note_outlined,
                                note, color: kSubtle)),
                      if (staffList.isNotEmpty) ...[
                        ...
                        Padding(
                          ...
                          child: Column(
                            children:
                                staffList.take(5).map((e) {
                              // ✅ FIXED armed/unarmed in district list preview
                              final eArmed =
                                  _staffIsArmed(e as Map);
                              return Padding(
                                padding: const EdgeInsets.symmetric(
                                    vertical: 3),
                                child: Row(children: [
                                  ...
                                  const SizedBox(width: 6),
                                  // ✅ armed icon in district preview
                                  Icon(
                                      eArmed
  ...
  String _buildCountLabel() {
    final parts = <String>[];
    if (_rankFilter != null)
      parts.add('पद: $_rankFilter');
    if (_armedFilter != _ArmedFilter.all)
      parts.add(_armedLabel(_armedFilter));
    if (_downloadFilter != _DownloadFilter.all)
      parts.add(_dlLabel(_downloadFilter));
  ...
  String _buildEmptyLabel() {
    ...
    if (_downloadFilter != _DownloadFilter.all)
      parts.add('"${_dlLabel(_downloadFilter)}"');
    if (parts.isNotEmpty) {
      return '${parts.join(' + ')} के लिए कोई ड्यूटी नहीं';
    }
...
// ══════════════════════════════════════════════════════════════════════════════
//  ARMED BADGE WIDGET
//  ✅ Single source of truth for the armed/unarmed pill on cards
// ══════════════════════════════════════════════════════════════════════════════
class _ArmedBadge extends StatelessWidget {
  final bool isArmed;
  const _ArmedBadge({required this.isArmed});
  ...
  Widget build(BuildContext context) {
    ...
    final icon  = isArmed ? Icons.shield_outlined : Icons.person_outline;
