...
// ─────────────────────────────────────────────
class _BP {
  static bool isPhone(BuildContext ctx)  => MediaQuery.of(ctx).size.width < 480;
  static bool isTablet(BuildContext ctx) => MediaQuery.of(ctx).size.width < 768;
  static bool isWide(BuildContext ctx)   => MediaQuery.of(ctx).size.width >= 1024;

  static double pad(BuildContext ctx) =>
      isPhone(ctx) ? 10 : isTablet(ctx) ? 14 : 18;

  static int statCols(double width) {
...
const List<String> kStates = ['उत्तर प्रदेश'];
...
const List<String> kElectionPhases = [
  ...
  'चतुर्थ चरण', 'पंचम चरण', 'षष्ठम चरण', 'सप्तम चरण',
];
...
class SuperAdminModel {
  final int      id;
  final String   name, username, district;
  ...
  SuperAdminModel({
    ...
    required this.username, required this.district,
    required this.createdAt,required this.adminsUnder,
...
class AdminModel {
  final int      id;
  final String   name, username, district, createdBy;
  final bool     isActive;
  final DateTime createdAt;
  ...
  AdminModel({
    ...
    required this.createdBy,      required this.superZoneCount,
  });
...
class ElectionConfigModel {
  final int    id;
  final String district, state, electionType, electionName,
               phase, electionYear, electionDate,
               pratahSamay, sayaSamay, instructions;
  final bool   isActive, isArchived;
  final String? archivedAt, createdAt;

  ElectionConfigModel({
    required this.id,             required this.district,
    required this.state,          required this.electionType,
    required this.electionName,   required this.phase,
    required this.electionYear,   required this.electionDate,
    required this.pratahSamay,    required this.sayaSamay,
    required this.instructions,   required this.isActive,
    required this.isArchived,
    this.archivedAt, this.createdAt,
  });
...
class SystemLogEntry {
  final int id; final String level, message, module; final DateTime time;
  SystemLogEntry({required this.id, required this.level,
    required this.message, required this.module, required this.time});
  factory SystemLogEntry.fromJson(Map<String, dynamic> j) => SystemLogEntry(
    id:      j['id']     ?? 0, level:   j['level']   ?? 'INFO',
    message: j['message'] ?? '', module: j['module']  ?? '',
    time:    DateTime.tryParse(j['time'] ?? '') ?? DateTime.now(),
  );
}

class ApiLogEntry {
  final int    id, statusCode, durationMs;
  final String method, path, username, role, ipAddress,
               requestBody, errorMessage, level;
  final DateTime createdAt;

  ApiLogEntry({
    required this.id,           required this.method,
    required this.path,         required this.statusCode,
    required this.durationMs,   required this.username,
    required this.role,         required this.ipAddress,
    required this.requestBody,  required this.errorMessage,
    required this.level,        required this.createdAt,
  });
...
class OverviewStats {
  final int totalSuperAdmins, totalAdmins, totalStaff,
            totalBooths, assignedDuties,
            activeElectionConfigs, archivedElectionConfigs;

  OverviewStats({
    this.totalSuperAdmins = 0, this.totalAdmins = 0, this.totalStaff = 0,
    this.totalBooths = 0,      this.assignedDuties = 0,
    this.activeElectionConfigs = 0, this.archivedElectionConfigs = 0,
  });
...
class _MasterDashboardState extends State<MasterDashboard>
  ...
  Future<void> _fetchAll() => Future.wait([
    _fetchOverview(),    _fetchSuperAdmins(),
  ...
  Future<void> _fetchSuperAdmins() async {
    ...
    try {
      ...
      _snack("सुपर एडमिन लोड करने में विफल", kError);
    } finally {
  ...
  Future<void> _fetchLogs({String level = 'ALL'}) async {
    ...
    try {
      ...
      _snack("लॉग लोड करने में विफल", kError);
    } finally {
      if (mounted) setState(() => _loadingLogs = false);
    }
  }

  // ── FIX: restored correct _fetchApiLogs — no widget.district/role refs ──
  Future<void> _fetchApiLogs() async {
    ...
    try {
      ...
      final params = <String, String>{
        ...
        if (_apiLogQueryCtrl.text.trim().isNotEmpty)
          'q': _apiLogQueryCtrl.text.trim(),
      };
      ...
      _snack("API लॉग लोड करने में विफल", kError);
    } finally {
  ...
  // ══════════════════════════════════════════
  //  ★ DISTRICT PICKER — for master navigation
  // ══════════════════════════════════════════
  void _showDistrictPickerThen({
    required String title,
    required IconData icon,
    required void Function(String district) onSelected,
  }) {
    String? picked;
    final TextEditingController searchCtrl = TextEditingController();
    List<String> filtered = List.from(kUpDistricts);
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            picked != null
                                ? 'चयनित: $picked'
                                : '${filtered.length} जनपद उपलब्ध',
                            style: TextStyle(
                              color: picked != null ? kDevAccent : kSubtle,
                              fontSize: 11.5,
                    ...
                    ConstrainedBox(
                      ...
                      child: filtered.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (_, i) {
                                final d      = filtered[i];
                                final isSel  = picked == d;
                                return InkWell(
                                  onTap: () => setDlg(() => picked = d),
  ...
  void _showCreateOrEditElectionConfig({ElectionConfigModel? existing}) {
    ...
    String? selectedState    =
        existing?.state.isNotEmpty == true ? existing!.state : kStates.first;
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => _styledDialog(
          title: isEdit ? 'निर्वाचन कॉन्फ़िग संपादित करें'
                        : 'नई निर्वाचन कॉन्फ़िग जोड़ें',
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Opacity(
                    ...
                    child: DropdownButtonFormField<String>(
                      ...
                      decoration: _dlgDecoration(
                          'जनपद *', Icons.location_city_outlined),
                      items: kUpDistricts
                      ...
                      validator: (v) =>
                          v == null || v.isEmpty ? 'जनपद चुनें' : null,
                    ),
                  ),
                ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  validator: (v) =>
                      v == null || v.isEmpty ? 'राज्य चुनें' : null,
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  value: selectedType,
                  ...
                  dropdownColor: Colors.white,
                  decoration: _dlgDecoration(
                      'निर्वाचन प्रकार *', Icons.how_to_vote_outlined),
                  ...
                  validator: (v) =>
                      v == null || v.isEmpty ? 'प्रकार चुनें' : null,
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  value: selectedPhase,
                  ...
                  dropdownColor: Colors.white,
                  decoration:
                      _dlgDecoration('चरण *', Icons.flag_outlined),
                  ...
                  validator: (v) =>
                      v == null || v.isEmpty ? 'चरण चुनें' : null,
                ),
                const SizedBox(height: 12),
                ...
                LayoutBuilder(builder: (ctx, c) {
                  ...
                  return Column(children: [
                    _timePickerField(context: context, ctrl: pratahCtrl,
                        label: 'प्रातः समय',
                        icon: Icons.wb_sunny_outlined),
                    const SizedBox(height: 12),
                    _timePickerField(context: context, ctrl: sayaCtrl,
                        label: 'सायं समय',
                        icon: Icons.nights_stay_outlined),
                  ]);
                }),
                const SizedBox(height: 12),
                _dlgField(instrCtrl,
                    'विशेष निर्देश (वैकल्पिक)', Icons.notes_outlined,
                    maxLines: 3),
                  ...
                  onConfirm: () async {
                    ...
                    try {
                      ...
                      final body  = {
                        ...
                        "electionName":
                            "$selectedType ${yearCtrl.text.trim()}",
                        "phase":        selectedPhase,
  ...
  Widget _timePickerField({
    ...
    return GestureDetector(
      ...
      child: AbsorbPointer(child: _dlgField(ctrl, label, icon)),
    );
  }
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  dropdownColor: Colors.white,
                  decoration:
                      _dlgDecoration('जनपद', Icons.location_city_outlined),
                  items: kUpDistricts
  ...
  void _showForceLogout() {
    final Map<String, bool> selected = {
      'super_admin': false, 'admin': false, 'staff': false,
    };
              ...
              _dlgActions(
                ...
                onConfirm: () async {
                  ...
                  final ok = await showDialog<bool>(
                    ...
                    builder: (c) => AlertDialog(
                      ...
                      content: Text(
                        ...
                        style:
                            const TextStyle(color: kDark, fontSize: 13),
                      ),
                      actions: [
                        TextButton(
                  ...
                  try {
                    ...
                    if (ctx.mounted) Navigator.pop(ctx);
                    _snack('${roles.length} भूमिका(एं) लॉगआउट हुईं ✓',
                        kSuccess);
                  } catch (e) {
                  ...
                  onConfirm: () async {
                    ...
                    try {
                      ...
                      await ApiService.patch(
                          endpoint, {"password": passCtrl.text},
                          token: token);
                      if (ctx.mounted) Navigator.pop(ctx);
        ...
        child: Column(children: [
          _dbToolTile(Icons.backup_outlined, 'बैकअप बनाएँ',
            ...
            try {
              ...
              _fetchSystemStats();
            } catch (_) { _snack('बैकअप विफल', kError); }
          }),
          ...
          _dbToolTile(Icons.cleaning_services_outlined, 'कैश साफ़ करें',
            ...
            try {
              final token = await AuthService.getToken();
              await ApiService.post(
                  "/master/db/flush-cache", {}, token: token);
              _snack('कैश साफ़ हुआ ✓', kInfo);
            } catch (_) { _snack('कैश साफ़ करने में विफल', kError); }
          }),
  ...
  void _confirmDestructive(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content:
            Text(body, style: const TextStyle(color: kDark, fontSize: 13)),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx),
              child: const Text('रद्द करें',
  ...
  // ── TOP BAR ───────────────────────────────
  Widget _buildTopBar() {
    final isPhone = _BP.isPhone(context);
    return Container(
      ...
      child: Row(children: [
        Container(
          padding: EdgeInsets.symmetric(
              horizontal: isPhone ? 6 : 8, vertical: 4),
          decoration: BoxDecoration(
              color: kDevAccent, borderRadius: BorderRadius.circular(6)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.terminal, color: Colors.white, size: 12),
            ...
            children: [
              Text('मास्टर एडमिन कंसोल',
                  style: TextStyle(
                      ...
                      letterSpacing: 1.2)),
              if (!isPhone)
                const Text('Election Management — Developer Access',
        ...
        if (!isPhone) ...[
          ...
          IconButton(
            ...
            icon: const Icon(Icons.storage, color: Colors.white54, size: 18),
            padding: EdgeInsets.zero,
            constraints: const BoxConstraints(minWidth: 30, minHeight: 30),
          ),
          IconButton(
            onPressed: () { _fetchAll(); _snack('रिफ्रेश…', kInfo); },
          ...
          children: List.generate(tabs.length, (i) {
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Row(children: [
                  Icon(tabs[i].$1,
                      size: 14, color: sel ? kDevAccent : kSubtle),
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          LayoutBuilder(builder: (ctx, c) {
            final cols = _BP.statCols(c.maxWidth);
            return GridView.count(
              shrinkWrap: true,
              crossAxisCount: cols,
              crossAxisSpacing: 10,
              mainAxisSpacing: 10,
              childAspectRatio: _BP.isPhone(context) ? 1.3 : 1.4,
          ...
          // ── ★ HIERARCHY REPORT — district picker ──────────────────
          _NavTileWithDistrict(
            label: 'पदानुक्रम रिपोर्ट',
            subtitle: 'जनपद चुनकर रिपोर्ट देखें',
            icon: Icons.table_chart_outlined,
            gradient: const [Color(0xFF0F2B5B), Color(0xFF1A3D7C)],
            districtIcon: Icons.table_chart_outlined,
            districtTitle: 'पदानुक्रम रिपोर्ट — जनपद चुनें',
            onDistrictSelected: (d) => Navigator.push(context,
              MaterialPageRoute(
                builder: (_) =>
                    HierarchyReportPage(role: "master", district: d),
              )),
            onShowPicker: _showDistrictPickerThen,
          ),
          const SizedBox(height: 12),

          // ── ★ MAP VIEW — district picker ───────────────────────────
          // Passes both district (selected by picker) AND role:"master"
          // so MapViewPage can filter hierarchy data and API calls correctly.
          _NavTileWithDistrict(
            label: 'मानचित्र दृश्य',
            subtitle: 'जनपद चुनकर मानचित्र देखें',
            icon: Icons.map,
            gradient: const [Color(0xFF00695C), Color(0xFF00897B)],
            districtIcon: Icons.map,
            districtTitle: 'मानचित्र दृश्य — जनपद चुनें',
            onDistrictSelected: (d) => Navigator.push(context,
              MaterialPageRoute(
                builder: (_) => MapViewPage(district: d, role: "master"),
              )),
            onShowPicker: _showDistrictPickerThen,
          ),
          const SizedBox(height: 18),
  ...
  Widget _electionSummaryBanner() {
    final active =
        _electionConfigs.where((c) => c.isActive && !c.isArchived).toList();
    return Container(
      decoration: BoxDecoration(
      ...
      padding: EdgeInsets.all(_BP.isPhone(context) ? 12 : 16),
      child: Row(children: [
    ...
    return Column(children: [
      Container(
        ...
        child: Row(children: [
          Expanded(
            child: Text(
              '${_electionConfigs.where((c) => !c.isArchived).length}'
              ' सक्रिय  •  '
          ...
          IconButton(
            tooltip:
                _showArchivedConfigs ? 'इतिहास छुपाएँ' : 'इतिहास दिखाएँ',
            onPressed: () {
              setState(() => _showArchivedConfigs = !_showArchivedConfigs);
              _fetchElectionConfigs();
            },
            icon: Icon(
              _showArchivedConfigs
                  ? Icons.visibility_off : Icons.history,
              size: 18, color: kSubtle),
            padding: EdgeInsets.zero,
            constraints: const BoxConstraints(minWidth: 32, minHeight: 32),
          ),
  ...
  Widget _electionConfigCard(ElectionConfigModel cfg) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isHistory
              ? kSubtle.withOpacity(0.5) : kDevAccent.withOpacity(0.5),
          width: 1.2,
        ),
        boxShadow: [BoxShadow(
            color: kPrimary.withOpacity(0.05), blurRadius: 8,
      ...
      child: Column(children: [
        Container(
          ...
          child: Row(children: [
            Icon(isHistory ? Icons.archive : Icons.how_to_vote,
            ...
            if (isHistory)
              _statusChip('इतिहास', kSubtle)
            else
              _statusChip('सक्रिय', kSuccess),
            const SizedBox(width: 4),
            if (!isHistory)
              PopupMenuButton<String>(
                onSelected: (v) {
                  if (v == 'edit')
                    _showCreateOrEditElectionConfig(existing: cfg);
                  if (v == 'archive') {
                    _confirmDestructive(
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            if (cfg.instructions.isNotEmpty)
              _kvRow(Icons.notes_outlined, 'निर्देश', cfg.instructions,
                  multiline: true),
            if (isHistory && cfg.archivedAt != null)
              _kvRow(Icons.archive_outlined, 'आर्काइव',
                  cfg.archivedAt!.split('T').first),
          ]),
        ),
      ]),
    );
      ...
      Expanded(
        child: _loadingSuperAdmins
            ? const Center(
                child: CircularProgressIndicator(color: kDevAccent))
            : RefreshIndicator(
                onRefresh: _fetchSuperAdmins, color: kDevAccent,
                child: _superAdmins.isEmpty
                    ? _emptyState('कोई सुपर एडमिन नहीं', '"नया" पर टैप करें',
      ...
      child: Column(children: [
        Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(
                crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(sa.name, style: const TextStyle(
                  color: kDark, fontWeight: FontWeight.w700, fontSize: 13)),
            ...
            PopupMenuButton<String>(
              onSelected: (v) {
                if (v == 'reset')
                  _showResetPassword(sa.id, sa.name, 'super_admin');
                if (v == 'delete') {
                  _confirmDestructive(
                    'सुपर एडमिन हटाएँ?',
      ...
      Expanded(
        child: _loadingAdmins
            ? const Center(
                child: CircularProgressIndicator(color: kDevAccent))
            : RefreshIndicator(
                onRefresh: _fetchAdmins, color: kDevAccent,
                child: _admins.isEmpty
                    ? _emptyState('कोई एडमिन नहीं', '"नया" पर टैप करें',
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(child: Column(
              ...
              const SizedBox(height: 4),
              _infoRowWidget(
                  Icons.person_outline, 'द्वारा: ${admin.createdBy}'),
              const SizedBox(height: 4),
              _infoRowWidget(Icons.calendar_today_outlined,
  ...
  Widget _buildApiLogs() {
    final totalPages =
        (_apiLogsTotal / _apiLogsLimit).ceil().clamp(1, 99999);
    return Column(children: [
      Container(
        color: kSurface,
        ...
        child: Row(children: [
          Expanded(
            child: TextField(
              ...
              onSubmitted: (_) { _apiLogsPage = 0; _fetchApiLogs(); },
      ...
      Container(
        ...
        child: SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          child: Row(children: [
            _filterDropdown(label: 'स्तर', value: _apiLogLevel,
                options: const ['ALL', 'INFO', 'WARN', 'ERROR'],
                onChanged: (v) {
                  setState(() => _apiLogLevel = v);
                  _apiLogsPage = 0; _fetchApiLogs();
                }),
            const SizedBox(width: 6),
            _filterDropdown(label: 'मेथड', value: _apiLogMethod,
            ...
            const SizedBox(width: 6),
            _filterDropdown(label: 'स्थिति', value: _apiLogStatus,
                options: const ['ALL','2xx','4xx','5xx',
                    '200','401','403','404','500'],
                onChanged: (v) {
            ...
            const SizedBox(width: 6),
            _filterDropdown(label: 'भूमिका', value: _apiLogRole,
      ...
      if (_apiLogsTotal > 0)
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            _pgBtn(Icons.first_page,    _apiLogsPage > 0,
                () { setState(() => _apiLogsPage = 0); _fetchApiLogs(); }),
            _pgBtn(Icons.chevron_left,  _apiLogsPage > 0,
                () { setState(() => _apiLogsPage--); _fetchApiLogs(); }),
            Container(
              margin: const EdgeInsets.symmetric(horizontal: 12),
              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
              decoration: BoxDecoration(
                  color: kDevAccent, borderRadius: BorderRadius.circular(20)),
              child: Text('${_apiLogsPage + 1} / $totalPages',
                  style: const TextStyle(
                      color: Colors.white, fontWeight: FontWeight.w800,
                      fontSize: 12)),
            ),
            _pgBtn(Icons.chevron_right, _apiLogsPage < totalPages - 1,
                () { setState(() => _apiLogsPage++); _fetchApiLogs(); }),
            _pgBtn(Icons.last_page,     _apiLogsPage < totalPages - 1,
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        DropdownButton<String>(
          value: value, underline: const SizedBox.shrink(),
          isDense: true, iconSize: 16,
          style: const TextStyle(
              color: kDark, fontSize: 11.5, fontWeight: FontWeight.w700),
          items: options
  ...
  Widget _apiLogTile(ApiLogEntry log) {
    ...
    return Container(
      ...
      child: ExpansionTile(
        ...
        dense: true, iconColor: kSubtle, collapsedIconColor: kSubtle,
        title: Row(children: [
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
            decoration: BoxDecoration(
                color: methodColor, borderRadius: BorderRadius.circular(4)),
        ...
        children: [
          if (log.errorMessage.isNotEmpty)
            _detailRow('त्रुटि', log.errorMessage,
                valueColor: kError, mono: true),
  ...
  Widget _detailRow(String k, String v,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          border: Border(
              top: BorderSide(color: kBorder.withOpacity(0.2)))),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        SizedBox(width: 60,
            child: Text(k, style: const TextStyle(
                color: kSubtle, fontSize: 10.5, fontWeight: FontWeight.w700))),
        Expanded(child: SelectableText(v,
            style: TextStyle(
                color: valueColor ?? kDark, fontSize: 11,
    ...
    return Column(children: [
      Container(
        ...
        child: SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          child: Row(
            children: ['ALL', 'INFO', 'WARN', 'ERROR'].map((f) =>
              GestureDetector(
                onTap: () {
      ...
      Expanded(
        child: _loadingLogs
            ...
            : RefreshIndicator(
                ...
                color: kDevAccent,
                child: filtered.isEmpty
                    ? _emptyState('कोई लॉग नहीं',
                        'इस फ़िल्टर के लिए कुछ नहीं',
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: BoxDecoration(
              color: color, borderRadius: BorderRadius.circular(4)),
          child: Text(log.level, style: const TextStyle(
  ...
  Widget _buildConfig() {
    ...
    return RefreshIndicator(
      onRefresh: _fetchConfig, color: kDevAccent,
      child: SingleChildScrollView(
        physics: const AlwaysScrollableScrollPhysics(),
        padding: EdgeInsets.all(_BP.pad(context)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 10),
          _configGroup(_appConfig.entries
          ...
          _configGroup([
            _devAction(Icons.archive_outlined, 'पुरानी कॉन्फ़िग आर्काइव करें',
              ...
              try {
                ...
                _fetchElectionConfigs(); _fetchOverview();
              } catch (_) { _snack('आर्काइव विफल', kError); }
            }),
            ...
            _devAction(Icons.lock_reset, 'मास्टर पासवर्ड बदलें',
                'मास्टर अकाउंट का पासवर्ड अपडेट करें',
                _showChangeMasterPassword),
            _devAction(Icons.delete_sweep_outlined,
                'पुराने API लॉग साफ़ करें',
          ...
          child: Form(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch,
              ...
              _dlgField(confCtrl, 'पुष्टि करें', Icons.lock_outline,
                  ...
                  validator: (v) =>
                      v != newCtrl.text ? 'पासवर्ड समान नहीं हैं' : null),
              const SizedBox(height: 20),
              _dlgActions(
                onCancel: () => Navigator.pop(ctx),
                onConfirm: () async {
                  ...
                  } catch (e) { _snack("त्रुटि: $e", kError); }
                },
                confirmLabel: 'बदलें',
              ),
            ]),
      ...
      child: Column(
        children: data.entries.toList().asMap().entries.map((e) {
          ...
          return Container(
            ...
            child: Row(children: [
              Text(kv.key, style: const TextStyle(
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: kSurface, borderRadius: BorderRadius.circular(6),
                  border: Border.all(color: kBorder.withOpacity(0.4)),
                ),
                child: Text(kv.value, style: const TextStyle(
  ...
  Widget _statusBadge(bool isActive) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: isActive
            ? kSuccess.withOpacity(0.1) : kError.withOpacity(0.1),
        borderRadius: BorderRadius.circular(20),
  ...
  Widget _configToggle(String title, String subtitle, bool value,
    ...
    return StatefulBuilder(builder: (ctx, setLocal) {
      ...
      return ListTile(
        ...
        trailing: Switch(
          value: val,
          onChanged: (v) { setLocal(() => val = v); onChanged(v); },
  ...
  Widget _configInfo(String key, String value) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 11),
      child: Row(children: [
        Expanded(child: Text(key, style: const TextStyle(
            color: kSubtle, fontSize: 12, fontWeight: FontWeight.w600))),
        const SizedBox(width: 8),
  ...
  Widget _styledDialog({
    ...
    required BuildContext ctx, required Widget child,
  }) {
    return Dialog(
      backgroundColor: Colors.transparent,
      insetPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 24),
  ...
  Widget _dlgField(
    ...
    return TextFormField(
      ...
      maxLines: obscure ? 1 : maxLines, keyboardType: keyboardType,
...
// ═══════════════════════════════════════════════════════════════════════
//  ★ NAV TILE WITH DISTRICT PICKER
// ═══════════════════════════════════════════════════════════════════════
class _NavTileWithDistrict extends StatelessWidget {
  final String   label, subtitle;
  final IconData icon;
  final List<Color> gradient;
  final IconData districtIcon;
  final String   districtTitle;
  final void Function(String) onDistrictSelected;
  final void Function({
    required String title,
    required IconData icon,
  ...
  const _NavTileWithDistrict({
    required this.label,             required this.subtitle,
    required this.icon,              required this.gradient,
    required this.districtIcon,      required this.districtTitle,
    required this.onDistrictSelected,required this.onShowPicker,
  });
